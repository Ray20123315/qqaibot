...
env.DB.m.delete(`rule_mute_enforcement:${gid}:${uid}`);env.DB.m.set(`ruleviolation:index:${gid}`,JSON.stringify(['rv3']));env.DB.m.set('ruleviolation:rv3',JSON.stringify({id:'rv3',groupId:gid,userId:uid,actionTaken:'record_only',actionsTaken:['recall','mute']}));ok((await m.findLatestActiveRuleViolationForUser(env,gid,uid))?.id==='rv3','active lookup');
