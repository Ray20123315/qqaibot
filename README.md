...
- **權限可勾選也可後改**：每個 capability 都會揭露是唯讀、寫入／修改、操作／執行或外部傳輸，並標示必要或可選。為保持舊插件相容，manifest 未宣告 `requiredCapabilities` 時沿用舊行為、視為全部必要；作者要提供可選權限時必須明確宣告 `requiredCapabilities`（可為空陣列）。可選權限不授權時，插件仍可啟用，但相依功能會由 Host capability gate 拒絕；必要權限缺少時才會 blocked。
