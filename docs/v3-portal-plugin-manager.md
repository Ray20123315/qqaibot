...
- Only a valid Portal session belonging to System Admin or a configured Developer may access `/api/portal/v3/plugins`.
- Other sessions receive HTTP 403 and the navigation entry stays hidden.
...
`injectV3PluginManagerClient()` adds a privileged 插件管理 view under the standalone 插件 navigation group without modifying the large legacy Portal runtime. The view is available to System Admin and configured Developers. The UI is generated from each plugin's normalized settings schema and supports boolean, number, string/secret, select, and JSON fields.
...
## Current lifecycle and package scope

The Plugin Manager controls plugins already assembled into the V3 runtime: persistent enable/disable, permission grants, release-channel preference, and plugin settings. Package metadata install/update/uninstall is handled separately by the V3 Package Manager. Package metadata operations do not hot-load or unload JavaScript runtime code; executable plugin code must already be a bundled or otherwise trusted runtime candidate.


## Lifecycle controls

The same System Admin/Developer Plugin Manager exposes persistent lifecycle controls backed by `plugin_lifecycle:registry:v1`:
