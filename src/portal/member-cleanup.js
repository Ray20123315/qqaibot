...
function injectMemberCleanupClient(html) {
  ...
  <div class="card cleanup-console">
    ...
    <div class="notice" id="cleanupStatus">尚未读取建议。</div>
    <div id="cleanupList" class="list"><div class="empty">尚无清人建议</div></div>
...
(function(){
  ...
  async function loadCleanup(){var ds=ce('memberDataStatus'),status=ce('cleanupStatus');if(ds)ds.textContent='正在读取成员资料…';if(status)status.textContent='正在读取清人建议…';var r=await cc('/members/cleanup');if(!r.ok){if(ds)ds.textContent=r.message||'读取失败';if(status)status.textContent=r.message||'读取失败';return}cleanupRecords=r.records||[];cleanupPermissions=r.permissions||{};applyPolicy(r.policy||{});renderSummary(r.summary||{});renderCleanup();renderMemberData();var meta=r.meta||{},time=Math.max(meta.fastSyncedAt||0,meta.deepSyncedAt||0);if(ds)ds.textContent='资料 '+cleanupRecords.length+' 人｜最近模式 '+String(meta.lastMode||'未同步')+'｜深度保留 '+String(meta.deepPreservedCount||0)+'｜群荣誉 '+String(meta.honorMemberCount||0)+' 人｜更新时间 '+cd(time)+(meta.honorSyncOk===false?'｜群荣誉接口失败：'+String(meta.honorSyncWarning||'未知错误'):'');if(status)status.textContent='建议已更新｜快照 '+cleanupRecords.length+' 人｜更新时间 '+cd(time)}
  ...
  window.qqaiLoadCleanup=loadCleanup;var oldLoad=window.qqaiLoadMembers;window.qqaiLoadMembers=async function(){if(typeof oldLoad==='function')await oldLoad();setTimeout(loadCleanup,0)};if(ce('v-member-data')&&ce('v-member-data').classList.contains('active'))setTimeout(loadCleanup,0)
})();
</script>`;
