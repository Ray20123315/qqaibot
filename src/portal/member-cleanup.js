...
function normalizeFullMember(raw, capturedAt = Date.now()) {
  ...
  return {
    ...
    sex: ["male", "female", "unknown"].includes(String(source.sex || "")) ? String(source.sex) : "unknown",
    age: Math.max(0, Math.min(200, Math.trunc(Number(source.age || 0)))),
    area: String(source.area || "").slice(0, 200),
    ...
    title,
    specialTitle: title,
    titleExpireTime: normalizeEpochMs(source.title_expire_time ?? source.titleExpireTime),
    ...
    rawFields: Object.keys(source).map(String).sort().slice(0, 100),
...
function honorMapFromResponse(response) {
  ...
  if (raw.current_talkative) add(raw.current_talkative, "current_talkative");
  for (const [field, type] of [
    ["talkative_list", "talkative"],
    ["performer_list", "performer"],
    ["legend_list", "legend"],
    ["strong_newbie_list", "strong_newbie"],
    ["emotion_list", "emotion"]
  ]) for (const entry of Array.isArray(raw[field]) ? raw[field] : []) add(entry, type);
  return map;
}
...
async function fetchHonors(env, groupId) {
  try {
    ...
    return honorMapFromResponse(response);
  } catch {
    return new Map();
  }
}
...
async function fastSync(env, groupId) {
  ...
  const snapshots = source.map(raw => ({ ...normalizeFullMember(raw, capturedAt), honors: honors.get(cleanId(raw?.user_id || raw?.userId || raw?.qq)) || [], syncMode: "fast" })).filter(item => item.qq);
  await writeSnapshots(env, groupId, snapshots);
  const meta = { groupId: cleanId(groupId), fastSyncedAt: capturedAt, deepSyncedAt: 0, memberCount: snapshots.length, honorMemberCount: snapshots.filter(item => item.honors?.length).length, lastMode: "fast" };
...
async function deepSync(env, groupId, userIds) {
  const ids = [...new Set((Array.isArray(userIds) ? userIds : []).map(cleanId).filter(Boolean))].slice(0, DEEP_BATCH_LIMIT);
  ...
  const results = await runPool(ids, 4, async userId => {
    try {
      ...
      const snapshot = { ...(existing.get(userId) || {}), ...normalizeFullMember(raw, capturedAt), honors: honors.get(userId) || existing.get(userId)?.honors || [], syncMode: "deep" };
  ...
  const meta = { ...previous, groupId: cleanId(groupId), deepSyncedAt: capturedAt, deepSyncedCount: Number(previous?.deepSyncedCount || 0) + updated.length, lastMode: "deep" };
  await dbPut(env, metaKey(groupId), JSON.stringify(meta));
...
function injectMemberCleanupClient(html) {
  ...
  const anchor = '<div id="memberList" class="list"><div class="empty">尚未读取群友列表</div></div>';
  ...
  <div class="card cleanup-console">
    ...
    <div class="cleanup-filters">
      <div class="field"><label>分类</label><select id="cleanupCategory"><option value="">全部</option><option value="cleanup_candidate">清理候选</option><option value="review">人工复核</option><option value="watch">观察</option><option value="keep">保留</option><option value="protected">受保护</option><option value="sync_first">资料不足</option></select></div>
      <div class="field"><label>搜索</label><input id="cleanupSearch" placeholder="昵称、群名片或 QQ"></div>
      <label class="member-toggle"><input id="cleanupHideProtected" type="checkbox" checked>隐藏受保护成员</label>
      <button id="cleanupSelectCandidates" class="btn ghost">选择全部候选</button>
      <button id="cleanupExport" class="btn ghost">导出完整 CSV</button>
      <button id="cleanupPreview" class="btn danger">建立清理预览</button>
    </div>
    <div class="notice" id="cleanupStatus">未同步。快速同步会读取成员列表与群荣誉；深度补全会逐人读取较可靠的地区、专属头衔等字段。</div>
...
.cleanup-console{margin-bottom:16px}.cleanup-head-actions,.cleanup-filters{display:flex;gap:8px;flex-wrap:wrap;align-items:end}.cleanup-summary{display:grid;grid-template-columns:repeat(6,minmax(100px,1fr));gap:10px;margin:12px 0}.cleanup-stat{padding:10px;border:1px solid var(--border);border-radius:8px}.cleanup-stat b{display:block;font-size:22px}.cleanup-policy{display:grid;grid-template-columns:repeat(4,minmax(120px,1fr)) auto auto auto;gap:10px;align-items:end;margin:12px 0}.cleanup-filters{margin:12px 0}.cleanup-filters .field{min-width:160px;margin:0}.cleanup-row{display:grid;grid-template-columns:auto minmax(190px,1.2fr) minmax(150px,.8fr) minmax(260px,1.4fr);gap:12px;align-items:start}.cleanup-score{font-size:20px;font-weight:800}.cleanup-reasons{font-size:12px;color:var(--muted);line-height:1.55}.cleanup-fields{font-size:12px;color:var(--muted);white-space:pre-wrap}.cleanup-execute{margin-top:12px;padding-top:12px;border-top:1px solid var(--border)}@media(max-width:1000px){.cleanup-summary,.cleanup-policy,.cleanup-row{grid-template-columns:1fr}.cleanup-head-actions .btn,.cleanup-filters .btn{flex:1 1 140px}}
</style>`;
...
(function(){
  ...
  function cd(value){var n=Number(value||0);return n?new Date(n).toLocaleString():'未知'}
  function cdays(value){return value==null?'未知':String(value)+' 天'}
  ...
  function renderCleanup(){var root=ce('cleanupList');if(!root)return;root.innerHTML=filtered().map(function(r){var m=r.member||{},c=r.classification||{},h=(m.honors||[]).map(function(x){return x.type}).join('、'),extra=Object.keys(m.extra||{}).map(function(k){return k+'='+String(m.extra[k])}).join('｜');var fields='群等级：'+(m.level||'未知')+'｜QQ等级：'+(m.qqLevel||0)+'｜专属头衔：'+(m.title||'无')+'｜头衔到期：'+cd(m.titleExpireTime)+'\\n地区：'+(m.area||'未知')+'｜年龄：'+(m.age||'未知')+'｜性别：'+(m.sex||'unknown')+'｜群荣誉：'+(h||'无')+(extra?'\\n扩展字段：'+extra:'');return'<div class="item cleanup-row"><label><input class="cleanup-select" type="checkbox" value="'+cs(m.qq)+'" '+((c.recommendation==='cleanup_candidate'||c.recommendation==='review')&&!c.protected?'':'disabled')+'></label><div><div class="member-name">'+cs(m.name||m.qq)+'</div><div class="member-meta">QQ '+cs(m.qq)+'｜'+cs(m.role||'member')+'｜入群 '+cs(cd(m.joinTime))+'｜最后发言 '+cs(cd(m.lastSentTime))+'</div><div class="cleanup-fields">'+cs(fields)+'</div></div><div><div class="cleanup-score">'+cs(c.score||0)+'</div><b>'+cs(c.label||c.category)+'</b><div class="member-meta">入群 '+cs(cdays(c.joinDays))+'｜未发言 '+cs(cdays(c.inactiveDays))+'</div></div><div class="cleanup-reasons">'+cs((c.reasons||[]).join('；')||'无分类理由')+'</div></div>'}).join('')||'<div class="empty">没有符合筛选条件的成员</div>'}
  async function loadCleanup(){var status=ce('cleanupStatus');if(status)status.textContent='正在读取分析资料…';var r=await cc('/members/cleanup');if(!r.ok){if(status)status.textContent=r.message||'读取失败';return}cleanupRecords=r.records||[];cleanupPermissions=r.permissions||{};applyPolicy(r.policy||{});renderSummary(r.summary||{});renderCleanup();if(status)status.textContent='分析完成｜快照 '+cleanupRecords.length+' 人｜最近模式 '+String(r.meta&&r.meta.lastMode||'未同步')+'｜更新时间 '+cd((r.meta&&Math.max(r.meta.fastSyncedAt||0,r.meta.deepSyncedAt||0))||0)}
  async function fastSync(){var r=await cc('/members/cleanup/sync','POST',{mode:'fast'});cn(r.message||'同步完成');if(r.ok)loadCleanup()}
  async function deepSync(){var ids=cselected();if(!ids.length){ids=cleanupRecords.filter(function(r){return !r.dataCompleteness||!r.dataCompleteness.deep}).slice(0,30).map(function(r){return r.member.qq})}if(!ids.length){cn('没有需要深度补全的成员');return}var r=await cc('/members/cleanup/sync','POST',{mode:'deep',userIds:ids.slice(0,30)});cn(r.message||'补全完成');if(r.ok)loadCleanup()}
