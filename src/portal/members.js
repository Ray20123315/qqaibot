...
function injectPortalMembersClient(html) {
  ...
  const section = `
<section id="v-member-data" class="view">
  <div class="section-head"><div><h2>成员资料</h2><p>群友名册、成员详细资料与清人建议集中在这里；不再拆成多个管理页面。</p></div><button id="memberRefresh" class="btn">刷新成员</button></div>
  <div class="card member-console-toolbar"><div class="field"><label for="memberSearch">搜索昵称或 QQ</label><input id="memberSearch" placeholder="输入昵称、群名片或 QQ"></div><div class="notice" id="memberConsoleStatus">请选择群组后读取成员列表。</div></div>
  <div class="card member-console-filters"><div class="field"><label for="memberRoleFilter">身份</label><select id="memberRoleFilter"><option value="">全部身份</option><option value="owner">群主</option><option value="admin">管理员</option><option value="member">普通成员</option></select></div><div class="field"><label for="memberMuteFilter">禁言状态</label><select id="memberMuteFilter"><option value="">全部状态</option><option value="muted">禁言中</option><option value="active">可发言</option></select></div><div class="field"><label for="memberSort">排序</label><select id="memberSort"><option value="role">身份优先</option><option value="name">名称</option><option value="recent">最近发言</option><option value="mute">剩余禁言</option></select></div><button id="memberResetFilters" class="btn ghost">重置筛选</button><button id="memberExport" class="btn ghost">导出 CSV</button></div>
  <div id="memberList" class="list"><div class="empty">尚未读取成员列表</div></div>
  <div id="memberHistoryPanel" class="card hidden"><div class="section-head compact"><div><h3 id="memberHistoryTitle">历史消息</h3><p>只显示本 Worker 已保存的该群聊天记录。</p></div><button id="memberHistoryClose" class="btn ghost">关闭</button></div><div id="memberHistoryList" class="list"><div class="empty">请选择成员</div></div></div>
  <div id="memberDataRoot"><div class="empty">尚未读取成员详细资料</div></div>
  <div id="memberCleanupRoot"><div class="empty">尚未读取清人建议</div></div>
</section>
...
(function(){
  ...
  function memberAreaActive(){return['v-member-data','v-member-actions'].some(function(id){var v=el(id);return v&&v.classList.contains('active')})}
  document.addEventListener('click',function(event){var target=event.target.closest&&event.target.closest('button');if(!target)return;var titles={memberActionsNav:'禁言管理',memberDataNav:'成员资料'};if(titles[target.id]){setTimeout(function(){var title=el('pageTitle');if(title)title.textContent=titles[target.id];loadMembers()},0)}else if(target.id==='memberRefresh'||target.id==='memberActionRefresh')loadMembers();else if(target.id==='memberExport')exportMembers();else if(target.id==='memberResetFilters'){['memberSearch','memberRoleFilter','memberMuteFilter','memberSort'].forEach(function(id){var node=el(id);if(node)node.value=id==='memberSort'?'role':''});renderMembers()}else if(target.classList.contains('member-copy'))copyText(target.dataset.qq);else if(target.id==='memberHistoryClose')el('memberHistoryPanel')&&el('memberHistoryPanel').classList.add('hidden');else if(target.classList.contains('member-history'))showHistory(target.dataset.qq);else if(target.classList.contains('member-mute'))muteMember(target);else if(target.classList.contains('member-unmute'))unmuteMember(target);});
  ...
  document.addEventListener('change',function(event){if(event.target&&['memberRoleFilter','memberMuteFilter','memberSort'].indexOf(event.target.id)>=0)renderMembers();if(event.target&&['memberActionRole','memberActionMute'].indexOf(event.target.id)>=0)renderMemberActions()});
