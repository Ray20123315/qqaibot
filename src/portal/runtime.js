  ...
  <main class="main">
    <header class="topbar">
      ...
      <div class="top-actions">
        <select id="groupSelect" aria-label="Group"><option value="">選擇群組</option></select>
    ...
    <div class="content">
      <section id="v-overview" class="view active">
        ...
        <div class="core-metrics">
          <article class="core-metric"><span>BOT</span><div><small>OneBot / NapCat</small><b id="mNapcat">讀取中…</b><em id="mNapcatSub">—</em></div></article>
          <article class="core-metric"><span>AI</span><div><small>AI runtime</small><b><strong id="mActive">0</strong> active</b><em><strong id="mQueued">0</strong> queued</em></div></article>
          <article class="core-metric"><span>!</span><div><small>Review</small><b><strong id="mProposals">0</strong> pending</b><em>permission gated</em></div></article>
        </div>
        <div class="core-grid">
          ...
          <div class="card"><div class="section-head compact"><div><h3 data-i18n="home.runtimeTitle">核心狀態</h3><p data-i18n="home.runtimeHelp">只顯示維持平台安全運作所需的資訊。</p></div><span id="overallStatus" class="status">—</span></div><div id="overviewIssues" class="list"><div class="empty" data-i18n="common.loading">讀取中…</div></div></div>
        </div>
      </section>
...
async function boot(attempt){attempt=Number(attempt||0);migratePortalMaintenanceV140();ensureR3Views();ensureAccountSecurityPanel();organizeSidebarNavigation();bindDashboardActions();applyPortalI18n();var me=await api('/me');if(!me.ok){if(me.retryable||me.code==='SESSION_STORAGE_UNAVAILABLE'){setNativeAdminVisibility(false);$('loginNotice').textContent=me.message||'登录会话资料库暂时不可用，正在重试…';if(attempt<3){setTimeout(function(){boot(attempt+1)},500*(attempt+1));return}showLogin();return}setNativeAdminVisibility(false);showLogin();return}showApp();session=me.session;loadAccountSecurity();$('identity').innerHTML='<b>'+esc(session.username||'admin')+'</b><br><span>'+esc(portalRoleLabel(session.role||'member'))+'</span>';if($('accountIdentitySummary'))$('accountIdentitySummary').innerHTML='<b>'+esc(session.username||'')+'</b><br><span>'+esc(portalRoleLabel(session.role||'member'))+'</span>';await loadPlugins();var loaded=await loadGroups();if(!loaded)return;await refreshCapabilities();applyR3RoleVisibility();applyPortalI18n();var hash=String(location.hash||'').replace(/^#/,'');if(hash&&$('v-'+hash)){var b=document.querySelector('#nav button[data-view="'+hash+'"]');if(b&&!b.hidden&&b.style.display!=='none')showView(hash);else showView('overview')}else showView('overview')}
