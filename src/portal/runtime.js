...
function legacyPhrase(source){var map=PORTAL_I18N.legacyPhrases||{},row=map[String(source||'')]||null;if(!row)return null;return row[portalLocale]??row[PORTAL_I18N.fallbackLocale||'en']??row.en??null}
function localizeLegacyNode(root){if(!root)return;var walker=document.createTreeWalker(root,NodeFilter.SHOW_TEXT);var nodes=[];while(walker.nextNode())nodes.push(walker.currentNode);nodes.forEach(function(node){var source=node.__qqaiI18nSource||String(node.nodeValue||'').trim();if(!source)return;var translated=legacyPhrase(source);if(!translated)return;if(!node.__qqaiI18nSource)node.__qqaiI18nSource=source;var raw=String(node.nodeValue||''),lead=(raw.match(/^\s*/)||[''])[0],tail=(raw.match(/\s*$/)||[''])[0];node.nodeValue=lead+translated+tail});root.querySelectorAll&&root.querySelectorAll('[placeholder],[title],[aria-label]').forEach(function(el){['placeholder','title','aria-label'].forEach(function(attr){if(!el.hasAttribute(attr))return;var store='qqaiI18n'+attr.replace(/-([a-z])/g,function(_,c){return c.toUpperCase()}).replace(/^./,function(c){return c.toUpperCase()});var source=el.dataset[store]||el.getAttribute(attr)||'';var translated=legacyPhrase(source);if(!translated)return;el.dataset[store]=source;el.setAttribute(attr,translated)})})}
function installLegacyI18nObserver(){var app=$('app');if(!app||app.__qqaiI18nObserved)return;app.__qqaiI18nObserved=true;new MutationObserver(function(records){records.forEach(function(record){record.addedNodes.forEach(function(node){if(node.nodeType===1)localizeLegacyNode(node);else if(node.nodeType===3&&node.parentElement)localizeLegacyNode(node.parentElement)})})}).observe(app,{childList:true,subtree:true})}
...
function showApp(){if($('app'))$('app').classList.remove('hidden');closeMobileSidebar();syncResponsivePortal()}
...
$('themeToggle').addEventListener('click',toggleTheme);if($('accountThemeToggle'))$('accountThemeToggle').addEventListener('click',toggleTheme);if($('localeSelect'))$('localeSelect').addEventListener('change',function(){setPortalLocale(this.value)});if($('accountLocaleSelect'))$('accountLocaleSelect').addEventListener('change',function(){setPortalLocale(this.value)});installLegacyI18nObserver();applyPortalI18n();
