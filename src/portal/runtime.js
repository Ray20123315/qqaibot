...
function ensureR3Views(){
  ...
  if(!$('v-maintenance').dataset.ready){$('v-maintenance').dataset.ready='1';$('v-maintenance').innerHTML='<div class="section-head"><div><h2>系统维护</h2><p>开启后，一般使用者只会看到维护页面，并阻止进入 Portal 后端；System Admin 与 Developer 保留后台进入能力。</p></div><button id="portalMaintenanceReload" class="btn">重新读取</button></div><div class="grid"><div class="card span-7"><h3>维护模式</h3><label class="switch"><input id="portalMaintenanceEnabled" type="checkbox">启用全站 Portal 维护</label><div class="field"><label for="portalMaintenanceMessage">维护页面说明</label><textarea id="portalMaintenanceMessage" placeholder="系统正在维护中，请稍后再试。"></textarea></div><div class="row"><button id="portalMaintenanceSave" class="btn primary">保存维护状态</button></div><div id="portalMaintenanceState" class="notice" style="margin-top:12px">尚未读取。</div></div><div class="card span-5"><h3>维护期间访问规则</h3><div class="notice">一般使用者：Portal 页面显示维护画面；Portal API 返回 HTTP 503。<br>System Admin／Developer：登录后可继续进入完整后台进行维护。<br>QQ Bot／OneBot 运行路径不会因为 Portal 维护而自动停止。</div></div></div>';$('portalMaintenanceReload').onclick=loadPortalMaintenance;$('portalMaintenanceSave').onclick=savePortalMaintenance}
...
async function loadPortalMaintenance(){if(!$('portalMaintenanceState'))return;var r=await api('/maintenance');if(!r.ok){$('portalMaintenanceState').textContent=r.message||'维护状态读取失败。';return}var s=r.state||{};$('portalMaintenanceEnabled').checked=!!s.enabled;$('portalMaintenanceMessage').value=String(s.message||'系统正在维护中，请稍后再试。');$('portalMaintenanceState').textContent=s.enabled?'维护模式已开启：一般使用者会被维护页面拦截；System Admin 与 Developer 可继续进入后台。':'维护模式目前关闭。';}
async function savePortalMaintenance(){if(!$('portalMaintenanceEnabled'))return;var enabled=!!$('portalMaintenanceEnabled').checked,message=String($('portalMaintenanceMessage').value||'').trim();if(enabled&&!(await confirmModal('开启后，一般使用者将无法进入 Portal，Portal 后端请求也会返回维护状态；System Admin 与 Developer 仍可进入。','开启系统维护',{danger:true})))return;var b=$('portalMaintenanceSave');b.disabled=true;var r=await api('/maintenance','POST',{enabled:enabled,message:message});b.disabled=false;$('portalMaintenanceState').textContent=r.message||(r.ok?'保存完成。':'保存失败。');if(r.ok)await loadPortalMaintenance();}
