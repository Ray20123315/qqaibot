...
function t(key){var messages=PORTAL_I18N.messages||{},table=messages[portalLocale]||{},fallback=messages[PORTAL_I18N.fallbackLocale||'en']||messages.en||{};return table[key]??fallback[key]??key}
...
function applyPortalI18n(){document.documentElement.lang=portalLocale;document.querySelectorAll('[data-i18n]').forEach(function(el){el.textContent=t(el.dataset.i18n)});document.querySelectorAll('#nav button[data-view]').forEach(function(el){el.textContent=portalTitle(el.dataset.view)});fillLocaleSelect($('localeSelect'));fillLocaleSelect($('accountLocaleSelect'));updateThemeButtons();renderPluginCatalog();var active=document.querySelector('#nav button[data-view].active');if(active&&$('pageTitle'))$('pageTitle').textContent=portalTitle(active.dataset.view)}
function setPortalLocale(locale){portalLocale=normalizeUiLocale(locale);try{localStorage.setItem('qqai_locale',portalLocale)}catch(e){}applyPortalI18n()}window.addEventListener('languagechange',function(){try{if(!localStorage.getItem('qqai_locale')){portalLocale=detectPortalLocale();applyPortalI18n()}}catch(e){}})
...
function portalTitle(name){var key='view.'+String(name||'').trim();var translated=t(key);return translated===key?String(name||''):translated}
