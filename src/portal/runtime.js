...
var PORTAL_PLUGINS=Array.isArray(window.__QQAI_PORTAL_PLUGINS__)?window.__QQAI_PORTAL_PLUGINS__:[];
...
async function loadPlugins(){var r=await api('/plugins');if(!r.ok){toast(r.message||t('common.unavailable'));return false}PORTAL_PLUGINS=Array.isArray(r.plugins)?r.plugins:[];renderPluginCatalog();return true}
...
function renderPluginCatalog(){var dev=!!(session&&((session.permissions||{}).developer||session.role==='developer')),visible=PORTAL_PLUGINS.filter(function(p){return!(p.portal&&p.portal.developerOnly)||dev});function card(p,compact){var copy=pluginLocalizedCopy(p),enabled=p.enabled!==false,view=pluginOpenView(p),openDisabled=!enabled||!view,bridge=p.portal&&p.portal.legacyBridge,state='<span class="plugin-state '+(enabled?'on':'off')+'">'+esc(enabled?t('plugins.enabled'):t('plugins.disabled'))+'</span>',toggle=dev&&!compact?'<button type="button" class="btn ghost plugin-toggle" data-plugin-toggle="'+esc(p.id)+'" data-next-enabled="'+(enabled?'false':'true')+'">'+esc(enabled?t('plugins.disable'):t('plugins.enable'))+'</button>':'';return '<article class="plugin-card'+(compact?' compact':'')+(enabled?'':' disabled')+'"><div class="plugin-icon">'+esc((p.portal&&p.portal.icon)||'◇')+'</div><div class="plugin-copy"><div class="plugin-card-head"><h3>'+esc(copy.name)+'</h3>'+state+(bridge?'<span class="plugin-badge">'+esc(t('plugins.bridge'))+'</span>':'')+'</div><p>'+esc(copy.description)+'</p></div><div class="plugin-actions"><button type="button" class="btn'+(openDisabled?'':' primary')+'" data-plugin-open="'+esc(p.id)+'" '+(openDisabled?'disabled':'')+'>'+esc(openDisabled?t('common.unavailable'):t('plugins.open'))+'</button>'+toggle+'</div></article>'}var grid=$('pluginGrid');if(grid)grid.innerHTML=visible.map(function(p){return card(p,false)}).join('')||'<div class="empty">'+esc(t('common.unavailable'))+'</div>';var mini=$('overviewPluginGrid');if(mini)mini.innerHTML=visible.filter(function(p){return p.enabled!==false}).slice(0,4).map(function(p){return card(p,true)}).join('');document.querySelectorAll('[data-plugin-open]').forEach(function(b){b.onclick=function(){var p=PORTAL_PLUGINS.find(function(x){return x.id===b.dataset.pluginOpen}),view=pluginOpenView(p);if(view)showView(view)}});document.querySelectorAll('[data-plugin-toggle]').forEach(function(b){b.onclick=function(){togglePlugin(b.dataset.pluginToggle,b.dataset.nextEnabled==='true')}})}
