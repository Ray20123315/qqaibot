...
import { portalPluginCatalog } from "../plugins/runtime.js";
...
function portalViewAvailable(view){var b=document.querySelector('#nav button[data-view="'+String(view||'').replace(/"/g,'')+'"]');return!!(b&&!b.hidden&&b.style.display!=='none')}
function pluginOpenView(plugin){return((plugin&&plugin.portal&&plugin.portal.views)||[]).find(portalViewAvailable)||''}
function renderPluginCatalog(){var dev=!!(session&&((session.permissions||{}).developer||session.role==='developer')),visible=PORTAL_PLUGINS.filter(function(p){return!(p.portal&&p.portal.developerOnly)||dev});function card(p,compact){var copy=pluginLocalizedCopy(p),view=pluginOpenView(p),disabled=!view,bridge=p.portal&&p.portal.legacyBridge;return '<article class="plugin-card'+(compact?' compact':'')+(disabled?' disabled':'')+'"><div class="plugin-icon">'+esc((p.portal&&p.portal.icon)||'◇')+'</div><div class="plugin-copy"><div class="plugin-card-head"><h3>'+esc(copy.name)+'</h3>'+(bridge?'<span class="plugin-badge">'+esc(t('plugins.bridge'))+'</span>':'')+'</div><p>'+esc(copy.description)+'</p></div><button type="button" class="btn'+(disabled?'':' primary')+'" data-plugin-open="'+esc(p.id)+'" '+(disabled?'disabled':'')+'>'+esc(disabled?t('common.unavailable'):t('plugins.open'))+'</button></article>'}var grid=$('pluginGrid');if(grid)grid.innerHTML=visible.map(function(p){return card(p,false)}).join('')||'<div class="empty">'+esc(t('common.unavailable'))+'</div>';var mini=$('overviewPluginGrid');if(mini)mini.innerHTML=visible.slice(0,4).map(function(p){return card(p,true)}).join('');document.querySelectorAll('[data-plugin-open]').forEach(function(b){b.onclick=function(){var p=PORTAL_PLUGINS.find(function(x){return x.id===b.dataset.pluginOpen}),view=pluginOpenView(p);if(view)showView(view)}})}
...
function organizeSidebarNavigation(){var nav=$('nav');if(!nav||nav.dataset.grouped==='3')return;var compat=$('pluginCompatNav');if(!compat){compat=document.createElement('div');compat.id='pluginCompatNav';compat.className='plugin-compat-nav';compat.hidden=true;nav.appendChild(compat)}var core=['overview','plugins','account','health'];Array.from(nav.querySelectorAll('button[data-view]')).forEach(function(b){if(core.includes(b.dataset.view))nav.insertBefore(b,compat);else compat.appendChild(b)});nav.dataset.grouped='3';applyPortalI18n();refreshSidebarGroupVisibility()}
