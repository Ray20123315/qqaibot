...
function publicAuthPageStyles() {
  return String.raw`
:root{color-scheme:light;--bg:#f4f7fb;--panel:#fff;--panel2:#f8fafc;--text:#172033;--muted:#68748a;--line:#e3e8f0;--primary:#5b5bd6;--primary2:#7d70e8;--ok:#17845f;--bad:#c53d4d;font-family:Inter,"Noto Sans TC","Microsoft JhengHei",system-ui,sans-serif}
*{box-sizing:border-box}body{margin:0;min-height:100dvh;background:radial-gradient(circle at 10% 10%,rgba(91,91,214,.15),transparent 34%),radial-gradient(circle at 88% 82%,rgba(23,132,95,.10),transparent 35%),var(--bg);color:var(--text)}a{color:inherit}.shell{width:min(1120px,calc(100% - 32px));margin:0 auto}.top{display:flex;align-items:center;justify-content:space-between;padding:22px 0}.brand{display:flex;align-items:center;gap:11px;text-decoration:none;font-weight:900}.logo{width:42px;height:42px;border-radius:14px;display:grid;place-items:center;background:linear-gradient(135deg,var(--primary),var(--primary2));color:white;box-shadow:0 10px 26px rgba(91,91,214,.24)}.nav{display:flex;gap:10px;align-items:center}.btn{display:inline-flex;align-items:center;justify-content:center;border:0;border-radius:12px;padding:11px 16px;font-weight:800;text-decoration:none;cursor:pointer;background:var(--panel2);color:var(--text)}.btn.primary{background:var(--primary);color:white}.btn.ghost{border:1px solid var(--line);background:rgba(255,255,255,.68)}.hero{padding:70px 0 56px;display:grid;grid-template-columns:1.15fr .85fr;gap:48px;align-items:center}.eyebrow{font-size:12px;font-weight:900;letter-spacing:.12em;text-transform:uppercase;color:var(--primary);margin-bottom:12px}.hero h1{font-size:clamp(40px,6vw,72px);line-height:1.02;letter-spacing:-.055em;margin:0 0 20px}.hero p{font-size:18px;line-height:1.75;color:var(--muted);margin:0 0 26px;max-width:720px}.actions{display:flex;gap:12px;flex-wrap:wrap}.panel{background:rgba(255,255,255,.9);border:1px solid var(--line);border-radius:26px;padding:26px;box-shadow:0 24px 70px rgba(31,42,68,.10);backdrop-filter:blur(14px)}.cards{display:grid;gap:12px}.mini{border:1px solid var(--line);border-radius:16px;padding:15px;background:var(--panel)}.mini b{display:block;margin-bottom:5px}.mini span{color:var(--muted);font-size:13px;line-height:1.55}.features{display:grid;grid-template-columns:repeat(3,1fr);gap:16px;padding:18px 0 70px}.feature{background:var(--panel);border:1px solid var(--line);border-radius:18px;padding:20px}.feature h3{margin:0 0 8px;font-size:16px}.feature p{margin:0;color:var(--muted);line-height:1.65;font-size:14px}.auth-wrap{min-height:calc(100dvh - 86px);display:grid;place-items:center;padding:34px 0 56px}.auth-card{width:min(520px,100%);background:rgba(255,255,255,.94);border:1px solid var(--line);border-radius:26px;padding:30px;box-shadow:0 24px 70px rgba(31,42,68,.11)}.auth-card h1{margin:0 0 7px;font-size:29px;letter-spacing:-.035em}.auth-card>p{margin:0 0 20px;color:var(--muted);line-height:1.65}.field{display:grid;gap:7px;margin:14px 0}.field label{font-size:13px;font-weight:800}.field input,.field select{width:100%;border:1px solid var(--line);border-radius:12px;background:white;color:var(--text);padding:12px 13px;outline:none}.field input:focus,.field select:focus{border-color:var(--primary);box-shadow:0 0 0 3px rgba(91,91,214,.12)}.notice{margin-top:14px;padding:11px 13px;border-radius:12px;background:var(--panel2);color:var(--muted);font-size:13px;line-height:1.55}.error{color:var(--bad)}.success{color:var(--ok)}.row{display:flex;gap:10px;align-items:center;flex-wrap:wrap}.grow{flex:1}.muted{color:var(--muted)}.divider{height:1px;background:var(--line);margin:22px 0}.small{font-size:12px;color:var(--muted);line-height:1.55}.hidden{display:none!important}details{margin-top:18px;border:1px solid var(--line);border-radius:14px;padding:12px 14px;background:var(--panel2)}summary{cursor:pointer;font-weight:800}.footer{padding:24px 0 36px;color:var(--muted);font-size:12px;text-align:center}@media(max-width:820px){.hero{grid-template-columns:1fr;padding-top:42px}.features{grid-template-columns:1fr}.nav .hide-mobile{display:none}}@media(max-width:520px){.shell{width:min(100% - 20px,1120px)}.top{padding:14px 0}.auth-card{padding:22px;border-radius:20px}.hero h1{font-size:42px}}
`;
}
...
function getPortalLoginPage() {
  ...
  <button id="loginBtn" type="button" class="btn primary" style="width:100%">登入</button><div id="message" class="notice">第一次使用？請先到「首次啟用」用身份驗證 ID 建立帳號與密碼。</div>
  <div class="divider"></div><div class="row"><a class="btn ghost grow" href="/register">首次啟用／帳號復原</a><a class="btn ghost grow" href="/">回首頁</a></div></section></main><footer class="footer">登入後的實際權限由伺服器判定。</footer></div>
  <script>
  ...
  async function login(){const username=$('username').value.trim(),password=$('password').value;const factorVisible=!$('factorBox').classList.contains('hidden');$('loginBtn').disabled=true;$('message').textContent='驗證中…';const r=await post('/api/auth/login-password',{username,password,remember:$('remember').checked,factorType:factorVisible?$('factorType').value:'',factorCode:factorVisible?$('factorCode').value:''});$('loginBtn').disabled=false;if(r.requiresTwoFactor||r.code==='TWO_FACTOR_REQUIRED'){$('factorBox').classList.remove('hidden');$('factorCode').focus();$('message').textContent=r.message||'請完成第二因素驗證。';return}if(!r.ok){$('message').textContent=r.message||'登入失敗。';$('message').className='notice error';return}$('message').textContent='登入成功，正在進入控制中心…';$('message').className='notice success';location.assign(nextPath())}
...
function getPortalRegisterPage() {
  ...
  <button id="activate" type="button" class="btn primary" style="width:100%">驗證並建立帳號</button><div id="message" class="notice">你的 QQID 只作為首次身份驗證與後端權限身份錨點；日常登入使用上方建立的帳號。</div>
  ...
  <div class="divider"></div><div class="row"><a class="btn ghost grow" href="/login">已有帳號？登入</a><a class="btn ghost grow" href="/">回首頁</a></div></section></main><footer class="footer">首次啟用不會改變你的伺服器角色；角色仍由已驗證的底層身份判定。</footer></div>
  <script>
  ...
  async function activate(){const qq=$('qqid').value.replace(/\\D/g,''),code=$('code').value.replace(/\\D/g,''),username=$('username').value.trim(),password=$('password').value,confirm=$('confirm').value;if(password!==confirm){$('message').textContent='兩次輸入的密碼不一致。';$('message').className='notice error';return}$('activate').disabled=true;$('message').textContent='正在驗證並建立帳號…';const r=await post('/api/auth/register',{qq,code,username,password,remember:$('remember').checked});$('activate').disabled=false;if(!r.ok){$('message').textContent=r.message||'首次啟用失敗。';$('message').className='notice error';return}$('message').textContent='帳號建立完成，正在進入控制中心…';$('message').className='notice success';location.assign('/portal')}
