...
import { developerIds, isDeveloperId } from "../core/identity.js";
...
import { brandFaviconLink, brandLockupMarkup, brandLogoSvg, brandPublicStyle, brandThemeBootScript } from "./brand.js";
...
async function handlePortalApi(request, env, url) {
  ...
  const systemAdmin = String(session.username || "").trim().toLowerCase() === "admin";
  const path = url.pathname.replace("/api/portal", "");

  if (path === "/system/developers") {
    if (!systemAdmin) return jsonResponse({ ok: false, code: "SYSTEM_ADMIN_REQUIRED", message: "只有使用系統管理員帳號登入後，才能管理 Developer QQ。" }, 403);
    if (request.method === "GET") {
      try {
        const managedIds = await readPortalManagedDeveloperIds(env);
        const staticEnv = env.qqaiStaticDeveloperIds === undefined
          ? env
          : { DEVELOPER_IDS: env.qqaiStaticDeveloperIds };
        return jsonResponse({ ok: true, managedIds, configuredIds: developerIds(staticEnv) });
      } catch {
        return jsonResponse({ ok: false, code: "AUTH_STORAGE_UNAVAILABLE", message: "Developer QQ 清單暫時無法讀取，請稍後重試。" }, 503);
      }
    }
    if (request.method === "POST") {
      try {
        const ids = await writePortalManagedDeveloperIds(env, body.ids);
        await writeSystemAudit(env, { type: "portal_system_developers", actorId: session.qq, action: "update", developerIds: ids }).catch(() => {});
        return jsonResponse({ ok: true, managedIds: ids, message: "Developer QQ 清單已更新。" });
      } catch (error) {
        if (error?.code === "DEVELOPER_QQ_INVALID") return jsonResponse({ ok: false, code: error.code, message: "請只輸入 5 到 12 位數字的 QQ，每行一個。" }, 400);
        if (error?.code === "DEVELOPER_QQ_LIMIT") return jsonResponse({ ok: false, code: error.code, message: "最多可管理 50 個 Developer QQ。" }, 400);
        return jsonResponse({ ok: false, code: "AUTH_STORAGE_UNAVAILABLE", message: "Developer QQ 清單儲存失敗，請稍後重試。" }, 503);
      }
    }
    return jsonResponse({ ok: false, message: "不支援的 Developer QQ 管理操作。" }, 405);
  }
  ...
  if (request.method === "POST" && path === "/security/2fa/setup") {
    ...
    try {
      ...
      const adminCredentialConfig = portalAdminCredentialConfig(env);
      const adminPasswordManaged = systemAdmin && adminCredentialConfig.mode === "configured";
      if (!passwordRecord && !adminPasswordManaged) return jsonResponse({ ok: false, code: "PASSWORD_REQUIRED", message: "请先设置登录密码，再启用双因数验证。" }, 400);
      const currentPassword = String(body.currentPassword || "");
      const recent = Date.now() - Number(session.authenticatedAt || 0) <= 15 * 60 * 1000;
      const passwordVerified = adminPasswordManaged
        ? constantTimeEqual(currentPassword, adminCredentialConfig.password)
        : Boolean(passwordRecord && await verifyPortalPassword(currentPassword, passwordRecord));
      if (!recent && !passwordVerified) return jsonResponse({ ok: false, code: "REAUTHENTICATION_REQUIRED", message: "请输入当前密码后再设置双因数验证。" }, 403);
  ...
  if (request.method === "POST" && path === "/security/2fa/disable") {
    ...
    try {
      ...
      const passwordVerified = adminPasswordManaged
        ? constantTimeEqual(currentPassword, adminCredentialConfig.password)
        : Boolean(passwordRecord && await verifyPortalPassword(currentPassword, passwordRecord));
      if (!passwordVerified) return jsonResponse({ ok: false, message: "当前密码错误。" }, 403);
      const secret = await decryptPortalAuthSecret(env, record.secret);
  ...
  if (request.method === "POST" && path === "/select-group") {
    ...
    const role = systemAdmin ? "developer" : await resolvePortalRole(env, session.qq, groupId);
    ...
    await writePortalSession(env, token, session, Number(session.expiresAt || 0));
  ...
  const role = systemAdmin ? "developer" : (groupId ? await resolvePortalRole(env, session.qq, groupId) : (isDeveloperId(env, session.qq) ? "developer" : "member"));
  ...
  const portalIsDeveloper = systemAdmin || permissions.developer || isDeveloperId(env, authed.qq);
  ...
  if (request.method === "GET" && path === "/me") {
    return jsonResponse({
      ...
      session: authed,
      systemAdmin,
      canManagePlugins: Boolean(portalIsDeveloper),
...
说明：${item.result}`).catch(() => null);
  ...
  if (path === '/platform/features') {
    if (!portalIsDeveloper) return jsonResponse({ ok: false, message: '功能能力目錄僅開發者本人可見。' }, 403);
    if (request.method === 'GET') {
      const features = await listPlatformFeatures(env, { role: 'developer', query: url.searchParams.get('q') || '', includeHidden: true });
      return jsonResponse({ ok: true, total: PLATFORM_FEATURE_COUNT, visible: features.length, features, catalogOnly: true, message: '此頁列出能力名稱與實作方式，不會控制 Worker 執行行為。', deploymentMode: 'single_worker', paidCloudflareServices: false });
    }
    if (request.method === 'POST') return jsonResponse({ ok: false, code: 'CAPABILITY_CATALOG_READ_ONLY', message: '能力目錄為唯讀；這些項目尚未接入執行時功能閘門。' }, 405, { Allow: 'GET' });
  }
...
function decoratePublicPageI18n(html, page) {
  ...
  source = source.replace("</head>", brandFaviconLink() + brandThemeBootScript() + brandPublicStyle() + "</head>");
  source = source.replace(/<a class="brand" href="\/\">[\s\S]*?<\/a>/, brandLockupMarkup({ href: "/", subtitle: "AI · PLUGINS · BYOR" }));
  source = source.replace('<nav class="nav">', '<nav class="nav"><select id="publicLocale" class="public-locale" aria-label="Language"></select><button id="publicTheme" class="public-theme-toggle" type="button" aria-label="Theme">☾</button>');
  const replacements = [
  ...
  source = source.replace(/<footer class="footer">[\s\S]*?<\/footer>/, '<footer class="footer"><span data-i18n="footer.rights">© 2026 ray20123315. All rights reserved.</span><span>AI Control Center · aibot.ray2025.com</span></footer>');
  return source.replace("</body>", themeClient + publicI18nClientScript() + "</body>");
}

function getPublicLandingPage() {
  const css = publicAuthPageStyles();
  let html = "<!doctype html><html lang=\"zh-Hant-TW\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><meta name=\"theme-color\" content=\"#020714\"><title>AI Control Center</title><style>" + css + "</style></head><body class=\"public-home\">\n<div class=\"cyber-skyline\" aria-hidden=\"true\"></div>\n<div class=\"edge-motto\" aria-hidden=\"true\">YOUR AI<br>YOUR PLUGINS<br>YOUR RESOURCES<br>ALL IN ONE</div>\n<div class=\"shell\">\n<header class=\"top\">\n  <a class=\"brand\" href=\"/\"><span class=\"brand-mark\" aria-hidden=\"true\"><i></i><i></i></span><span class=\"brand-copy\"><b>AI Control Center</b><small>AI · Automation · BYOR</small></span></a>\n  <nav class=\"nav\">\n    <a class=\"active hide-tablet\" href=\"/\">首頁</a>\n    <a class=\"hide-tablet\" href=\"/login\">登入</a>\n    <a class=\"hide-tablet\" href=\"/register\">首次啟用</a>\n    <a class=\"hide-tablet\" href=\"#features\">插件中心</a>\n    <a class=\"hide-tablet\" href=\"https://github.com/Ray20123315/qqaibot#readme\" target=\"_blank\" rel=\"noreferrer\">文件說明</a>\n    <a class=\"nav-cta\" href=\"/login\">立即開始&nbsp; →</a>\n  </nav>\n</header>\n<main>\n<section class=\"hero\">\n  <div class=\"hero-copy\">\n    <h1>把 AI、插件與自己的資源<br>放在<span class=\"gradient\">同一個控制中心</span></h1>\n    <p>安全帳密、權限與插件執行留在核心；其他能力按需啟用。</p>\n    <div class=\"hero-actions\"><a class=\"btn primary\" href=\"/login\">登入控制中心 <span>→</span></a><a class=\"btn ghost\" href=\"/register\">第一次使用？建立帳號</a></div>\n    <div class=\"trust-row\">\n      <div class=\"trust\"><span class=\"trust-icon\">◇</span><span><b>安全可靠</b><small>帳密 · 2FA · 權限隔離</small></span></div>\n      <div class=\"trust\"><span class=\"trust-icon\">ϟ</span><span><b>靈活擴展</b><small>插件與外部 API</small></span></div>\n      <div class=\"trust\"><span class=\"trust-icon\">⬡</span><span><b>自己的資源</b><small>BYOR · 成本自己掌控</small></span></div>\n    </div>\n  </div>\n\n  <div class=\"console-preview\" aria-label=\"AI Control Center 功能預覽\">\n    <div class=\"preview-top\">\n      <div class=\"preview-brand\"><span class=\"brand-mark\"><i></i><i></i></span>AI Control Center</div>\n      <div class=\"preview-search\">搜尋插件、資源或任務...</div>\n      <div class=\"preview-tools\"><span class=\"preview-tool\">i</span><span class=\"preview-avatar\">A</span></div>\n    </div>\n    <div class=\"preview-shell\">\n      <div class=\"preview-nav\"><span class=\"active\">總覽</span><span>AI 助理</span><span>插件中心</span><span>資源管理</span><span>工作流程</span><span>任務紀錄</span><span>團隊與權限</span><span>系統設定</span></div>\n      <div class=\"preview-main\">\n        <div class=\"preview-welcome\"><div class=\"preview-kicker\">CONTROL CENTER</div><h3>早安，<br>歡迎回到 <strong>AI Control Center</strong></h3><p>連接一切，讓 AI 為你工作。以下為能力預覽，登入後讀取真實狀態。</p></div>\n        <div class=\"preview-main-grid\">\n          <div class=\"preview-core\">\n            <div class=\"preview-stat-grid\">\n              <div class=\"preview-stat\" style=\"--stat-accent:#26b9ff\"><small>執行中的任務</small><b>登入後讀取</b><em>● 即時狀態</em></div>\n              <div class=\"preview-stat\" style=\"--stat-accent:#8a5cff\"><small>已安裝插件</small><b>依實例顯示</b><em>● 不使用假數字</em></div>\n              <div class=\"preview-stat\" style=\"--stat-accent:#1fd9c0\"><small>已連接資源</small><b>BYOR</b><em>● 由你的帳號接入</em></div>\n              <div class=\"preview-stat\" style=\"--stat-accent:#4d7fff\"><small>API 呼叫</small><b>動態統計</b><em>● 依 runtime 記錄</em></div>\n            </div>\n            <div class=\"flow\">\n              <div class=\"flow-head\"><b>AI 工作流程</b><span>● 可組合</span></div>\n              <div class=\"flow-nodes\"><div class=\"flow-node\"><b>觸發事件</b>Webhook / 排程</div><div class=\"flow-node\"><b>AI 處理</b>LLM / 推理</div><div class=\"flow-node\"><b>插件執行</b>受控能力</div><div class=\"flow-node\"><b>輸出結果</b>儲存 / 通知 / API</div></div>\n            </div>\n          </div>\n          <aside class=\"preview-resource-rail\">\n            <div class=\"preview-resource-head\"><b>資源連接狀態</b><span>能力預覽</span></div>\n            <div class=\"resource-row\"><span class=\"resource-logo\">D1</span><b>Cloudflare D1</b><span class=\"resource-state core\">核心</span></div>\n            <div class=\"resource-row\"><span class=\"resource-logo\">DO</span><b>Durable Object</b><span class=\"resource-state core\">核心</span></div>\n            <div class=\"resource-row\"><span class=\"resource-logo\">V</span><b>Vectorize</b><span class=\"resource-state optional\">可選</span></div>\n            <div class=\"resource-row\"><span class=\"resource-logo\">AI</span><b>Workers AI</b><span class=\"resource-state optional\">自備</span></div>\n            <div class=\"resource-row\"><span class=\"resource-logo\">KV</span><b>KV</b><span class=\"resource-state planned\">預留</span></div>\n            <div class=\"resource-row\"><span class=\"resource-logo\">R2</span><b>R2</b><span class=\"resource-state planned\">預留</span></div>\n            <div class=\"resource-row\"><span class=\"resource-logo\">API</span><b>外部 API</b><span class=\"resource-state ready\">自備</span></div>\n            <div class=\"preview-disclaimer\">KV / R2 目前為未來 BYOR 預留，不假裝已連接。</div>\n          </aside>\n        </div>\n      </div>\n    </div>\n  </div>\n</section>\n\n<section class=\"section\" id=\"features\">\n  <div class=\"section-headline\"><div><div class=\"section-kicker\">CORE CAPABILITIES</div><h2>核心功能</h2></div><p>從登入到資源接入，對應你這套平台現在與規劃中的實際能力。</p></div>\n  <div class=\"feature-grid\">\n    <article class=\"feature\" style=\"--accent:#31a7ff\"><div class=\"feature-icon\">ID</div><h3>帳號密碼登入</h3><p>一般使用者以帳號密碼登入，第二因素與復原流程完整保留。</p><div class=\"feature-foot\">ACCOUNT SECURITY →</div></article>\n    <article class=\"feature\" style=\"--accent:#805fff\"><div class=\"feature-icon\">✓</div><h3>保留 admin 開發者帳號</h3><p>開發者固定使用 admin，第一次直接設定密碼；一般使用者身份綁定流程仍保留。</p><div class=\"feature-foot\">FIRST ACTIVATION →</div></article>\n    <article class=\"feature\" style=\"--accent:#ad66ff\"><div class=\"feature-icon\">✚</div><h3>插件中心</h3><p>可信插件與外部 sandbox 分流，權限依需要授權，不共享核心 secrets。</p><div class=\"feature-foot\">PLUGIN ECOSYSTEM →</div></article>\n    <article class=\"feature\" style=\"--accent:#1fd8ff\"><div class=\"feature-icon\">DB</div><h3>BYOR / 自備資源</h3><p>D1、Vectorize、Workers AI 與外部 API 依部署者自己的 Cloudflare / API 資源接入。</p><div class=\"feature-foot\">YOUR RESOURCES →</div></article>\n    <article class=\"feature\" style=\"--accent:#ffb96c\"><div class=\"feature-icon\">CF</div><h3>Cloudflare 資源接入</h3><p>依功能逐項建立或選擇資源；KV / R2 等尚未進 source 的能力會明確標示預留。</p><div class=\"feature-foot\">RESOURCE ONBOARDING →</div></article>\n    <article class=\"feature\" style=\"--accent:#31ddb0\"><div class=\"feature-icon\">ACL</div><h3>角色與權限管理</h3><p>開發者、實例管理員與一般使用者的真正權限始終由伺服器判定。</p><div class=\"feature-foot\">SERVER AUTHORITY →</div></article>\n  </div>\n</section>\n\n<section class=\"section\" id=\"roles\">\n  <div class=\"section-headline\"><div><div class=\"section-kicker\">BUILT FOR DIFFERENT ROLES</div><h2>適合不同角色的使用場景</h2></div><p>同一個控制中心，依身份與授權只顯示真正能操作的功能。</p></div>\n  <div class=\"roles-layout\">\n    <div class=\"role-grid\">\n      <article class=\"role\"><div class=\"role-top\"><span class=\"role-icon\">&lt;/&gt;</span><h3>開發者</h3></div><p>維護核心、插件、部署、安全與所有系統層級設定。</p></article>\n      <article class=\"role\"><div class=\"role-top\"><span class=\"role-icon\">♕</span><h3>實例管理員</h3></div><p>管理自己的部署、插件與 BYOR 接入，不使用你的付費資源。</p></article>\n      <article class=\"role\"><div class=\"role-top\"><span class=\"role-icon\">AI</span><h3>AI 使用者</h3></div><p>使用自己的帳號體驗 AI 功能、個人設定與被授權的社群能力。</p></article>\n      <article class=\"role\"><div class=\"role-top\"><span class=\"role-icon\">◎</span><h3>社群成員</h3></div><p>參與已啟用的社群功能、插件與互動能力。</p></article>\n    </div>\n    <aside class=\"role-quote\"><p>讓每個人都能掌控屬於自己的 AI 力量，而不是共用一套不透明的資源與權限。</p><b>— AI CONTROL CENTER</b></aside>\n  </div>\n</section>\n\n<section class=\"cta-band\"><div><h2>YOUR AI · YOUR PLUGINS · YOUR RESOURCES</h2><p>先建立安全帳密，再把真正需要的能力一個一個接進來。</p></div><div class=\"hero-actions\"><a class=\"btn ghost\" href=\"/register\">首次啟用</a><a class=\"btn primary\" href=\"/login\">登入控制中心 →</a></div></section>\n</main>\n<footer class=\"footer\"><span>AI Control Center · AI-native automation platform</span><span>Simple · Powerful · Yours</span></footer>\n</div>\\n</body></html>";
  return decoratePublicPageI18n(html, "landing");
}
...
function getPortalHomePage(host) {
  const pluginCatalogJson = JSON.stringify(portalPluginCatalog()).replace(/</g, "\\u003c");
...
.login{min-height:100vh;display:grid;place-items:center;padding:28px;background:radial-gradient(circle at 12% 8%,rgba(91,91,214,.20),transparent 34%),radial-gradient(circle at 92% 82%,rgba(23,132,95,.12),transparent 34%),linear-gradient(145deg,var(--bg),var(--panel2))}
.login-card{width:min(1040px,100%);background:var(--login-panel);border:1px solid var(--line);border-radius:30px;box-shadow:0 30px 80px rgba(24,34,58,.14);backdrop-filter:blur(18px);overflow:hidden;display:grid;grid-template-columns:minmax(290px,.9fr) minmax(390px,1.1fr)}.login-aside{padding:36px;background:linear-gradient(155deg,rgba(91,91,214,.10),rgba(23,132,95,.05));border-right:1px solid var(--line);display:flex;flex-direction:column;gap:24px}.login-main{padding:34px 36px}.login-toolbar{display:flex;justify-content:flex-end;margin-bottom:10px}.brand{display:flex;align-items:center;gap:12px;margin-bottom:0}.logo{width:48px;height:48px;border-radius:16px;display:grid;place-items:center;background:linear-gradient(135deg,var(--primary),#8a69e8);color:#fff;font-weight:900;box-shadow:0 10px 24px rgba(91,91,214,.22)}.brand h1{font-size:24px;margin:0;letter-spacing:-.02em}.brand p{margin:5px 0 0;color:var(--muted);font-size:14px;line-height:1.55}.login-kicker{font-size:12px;font-weight:850;letter-spacing:.12em;text-transform:uppercase;color:var(--primary);margin-bottom:8px}.login-aside-copy h2{font-size:28px;line-height:1.18;margin:0 0 10px;letter-spacing:-.035em}.login-aside-copy p{margin:0;color:var(--muted);line-height:1.7}.login-role-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.login-role{border:1px solid var(--line);border-radius:15px;padding:13px;text-align:left;background:rgba(255,255,255,.45);color:var(--text);cursor:pointer;min-height:88px;transition:.18s ease}.login-role:hover{transform:translateY(-1px);border-color:rgba(91,91,214,.45)}:root[data-theme="dark"] .login-role{background:rgba(255,255,255,.035)}.login-role.active{border-color:var(--primary);box-shadow:0 0 0 3px rgba(91,91,214,.10);background:rgba(91,91,214,.08)}.login-role b{display:block;font-size:14px;margin-bottom:5px}.login-role small{display:block;color:var(--muted);font-size:12px;line-height:1.45}.login-profile-note{border:1px solid var(--line);border-radius:15px;padding:14px;background:var(--panel);font-size:13px;line-height:1.6}.login-profile-note b{display:block;margin-bottom:4px}.login-security-note{font-size:12px;color:var(--muted);line-height:1.55;margin-top:auto}.login-main-head h2{margin:0 0 6px;font-size:22px}.login-main-head p{margin:0 0 18px;color:var(--muted);font-size:14px;line-height:1.6}.field{display:grid;gap:7px;margin:14px 0}.field label{font-size:13px;font-weight:700}.field input,.field select,.field textarea{width:100%;border:1px solid var(--line);border-radius:12px;background:var(--panel);color:var(--text);padding:11px 12px;outline:none}.field input:focus,.field select:focus,.field textarea:focus{border-color:var(--primary);box-shadow:0 0 0 3px rgba(91,91,214,.12)}.field textarea{min-height:100px;resize:vertical}.row{display:flex;gap:10px;align-items:center;flex-wrap:wrap}.grow{flex:1;min-width:160px}
.btn{border:0;border-radius:11px;padding:10px 14px;font-weight:700;cursor:pointer;background:var(--panel2);color:var(--text)}.btn:hover{filter:brightness(.98)}.btn.primary{background:var(--primary);color:#fff}.btn.danger{background:#fde9ec;color:var(--bad)}.btn.ghost{background:transparent;border:1px solid var(--line)}.btn:disabled{opacity:.55;cursor:not-allowed}.notice{margin-top:14px;padding:11px 13px;border-radius:11px;background:var(--panel2);color:var(--muted);font-size:14px;line-height:1.55}.login-methods{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:0 0 14px}.login-methods .btn.active{background:var(--primary);color:#fff}.login-method-note{font-size:12px;color:var(--muted);margin:-4px 0 12px;line-height:1.55}.backup-codes{font-family:ui-monospace,SFMono-Regular,Consolas,monospace;white-space:pre-wrap;line-height:1.8}.security-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.security-grid>.card{box-shadow:none}@media(max-width:860px){.login{padding:16px}.login-card{grid-template-columns:1fr}.login-aside{border-right:0;border-bottom:1px solid var(--line);padding:26px}.login-main{padding:26px}.login-role-grid{grid-template-columns:1fr 1fr}}@media(max-width:560px){.login-role-grid{grid-template-columns:1fr}.login-aside-copy h2{font-size:24px}.login-main,.login-aside{padding:22px}.login-toolbar{margin-bottom:6px}.security-grid{grid-template-columns:1fr}}
  ...
  <div class="login-card">
    <div class="login-aside">
      ...
      <div class="login-role-grid" id="loginRoleGrid">
        ...
        <button type="button" class="login-role" data-login-profile="instance_admin"><b>實例管理員</b><small>管理自己的部署、Cloudflare 資源與插件。</small></button>
        <button type="button" class="login-role" data-login-profile="ai_user"><b>AI 使用者</b><small>直接使用 AI、個人設定與可用額度。</small></button>
        <button type="button" class="login-role" data-login-profile="community_member"><b>社群成員</b><small>所在社群已啟用服務，但不管理部署資源。</small></button>
      </div>
      <div class="login-profile-note"><b id="loginProfileTitle">開發者入口</b><span id="loginProfileDescription">建議使用密碼與第二因素登入；實際功能仍由伺服器權限判定。</span></div>
      <div class="login-security-note">安全提示：入口選擇不會改變任何權限。所有管理能力、資源存取與敏感操作仍由登入後的伺服器授權決定。</div>
    </div>
    <div class="login-main">
      <div class="login-toolbar"><button id="loginThemeToggle" type="button" class="btn ghost theme-toggle">切換顯示模式</button></div>
      <div class="login-main-head"><h2>登入你的控制中心</h2><p>可以使用訊息驗證碼，或使用已設定的密碼。開發者與管理員建議啟用第二因素。</p></div>
      <div class="field"><label for="loginQq">帳號／QQ ID</label><input id="loginQq" autocomplete="username" placeholder="密碼登入輸入帳號；驗證碼登入輸入 QQ ID"></div>
      <div class="login-methods"><button id="loginMethodCode" type="button" class="btn active">訊息驗證碼</button><button id="loginMethodPassword" type="button" class="btn">密碼登入</button></div>
      <div id="loginMethodNote" class="login-method-note">驗證碼會透過你已連接的訊息服務傳送。</div>
      <div id="loginCodePane">
        <div class="row"><button id="sendCode" type="button" class="btn ghost grow">傳送驗證碼</button></div>
        <div class="field"><label for="loginCode">六位驗證碼</label><input id="loginCode" inputmode="numeric" maxlength="6" autocomplete="one-time-code" placeholder="輸入六位驗證碼"></div>
        <button id="verifyCode" type="button" class="btn primary" style="width:100%">使用驗證碼登入</button>
      </div>
      <div id="loginPasswordPane" class="hidden">
        <div class="field"><label for="loginPassword">密碼</label><input id="loginPassword" type="password" maxlength="128" autocomplete="current-password" placeholder="輸入密碼"></div>
        <div id="loginFactorWrap" class="hidden">
          <div class="field"><label for="loginFactorType">第二因素</label><select id="loginFactorType"><option value="totp">驗證器動態碼</option><option value="backup">單次備用碼</option><option value="qq_code">訊息驗證碼</option></select></div>
          <div class="field"><label for="loginFactorCode">動態碼／備用碼／訊息驗證碼</label><input id="loginFactorCode" autocomplete="one-time-code" placeholder="輸入第二因素"></div>
          <button id="passwordSendFactorCode" type="button" class="btn ghost" style="width:100%">傳送訊息驗證碼作為第二因素</button>
        </div>
        <button id="verifyPassword" type="button" class="btn primary" style="width:100%;margin-top:12px">使用密碼登入</button>
        <details id="loginPasswordReset" class="settings-fold" style="margin-top:14px"><summary>安全／復原登入</summary><div class="settings-fold-body"><p class="muted">如果密碼無法使用，可先取得訊息驗證碼，再建立新的登入密碼；不需要舊密碼。</p><div class="field"><label for="loginResetPassword">新密碼（至少 10 個字元）</label><input id="loginResetPassword" type="password" maxlength="128" autocomplete="new-password"></div><div class="field"><label for="loginResetConfirm">確認新密碼</label><input id="loginResetConfirm" type="password" maxlength="128" autocomplete="new-password"></div><div class="field"><label for="loginResetCode">六位訊息驗證碼</label><input id="loginResetCode" inputmode="numeric" maxlength="6" autocomplete="one-time-code"></div><div class="row"><button id="passwordResetSendCode" type="button" class="btn ghost grow">傳送復原驗證碼</button><button id="passwordResetSubmit" type="button" class="btn primary grow">重設密碼</button></div></div></details>
      </div>
      <label class="switch"><input id="rememberLogin" type="checkbox" checked>在這台裝置保持登入（最長 180 天）</label>
      <div id="loginNotice" class="notice">登入憑證與備用碼以安全方式保存；持久登入使用 HttpOnly Cookie。若你沒有某項管理權限，登入後也不會顯示或開放該功能。</div>
    </div>
  </div>
</section>
<div id="app" class="app workspace-shell">
  <aside id="sidebar" class="sidebar workspace-sidebar">
    ...
    <div class="sidebar-principle"><span>SMALL CORE</span><b>Auth · ACL · Plugins</b><small>Product features stay outside the core.</small></div>
  ...
  <main class="main workspace-main">
    <header class="topbar workspace-topbar">
      <div class="workspace-heading">
        <button id="menu" class="btn ghost mobile-menu" aria-label="Menu">☰</button>
    ...
    <div class="content workspace-content">
      <section id="v-overview" class="view active core-home">
        <div class="workspace-hero">
          <div class="workspace-hero-logo">${brandLogoSvg("qqai-brand-logo portal-hero-logo")}</div>
          <div class="workspace-hero-copy"><div class="eyebrow" data-i18n="home.kicker">AI CONTROL CENTER</div><h1 data-i18n="home.title">保持核心簡單，把能力交給插件。</h1><p data-i18n="home.summary">登入、權限、安全與插件執行留在核心；其他能力由插件提供。</p><div class="workspace-hero-actions"><button class="btn primary" data-open-view="plugins" data-i18n="nav.plugins">插件</button><button class="btn ghost" data-open-view="account" data-i18n="nav.account">帳號與設定</button></div></div>
          <div class="workspace-core-map"><span>CORE</span><b>Authentication</b><b>Authorization</b><b>Plugin Runtime</b><small>Everything else is optional.</small></div>
        </div>
        ...
        <div class="workspace-principles">
          ...
          <article><span>02</span><b>Disabled by default</b><p>Product plugins are enabled only when needed.</p></article>
          <article><span>03</span><b>BYOR</b><p>D1 / AI / APIs remain deployment-owned resources.</p></article>
        </div>
      </section>
      <section id="v-plugins" class="view plugin-center">
        <div class="section-head"><div><div class="top-kicker">FEATURE EXTENSIONS</div><h2 data-i18n="plugins.title">插件中心</h2><p data-i18n="plugins.subtitle">非必要能力以插件呈現；依需要啟用。</p></div></div>
        <div id="pluginManagementHint" class="plugin-management-hint">正在確認插件管理權限…</div>
        <div id="pluginGrid" class="plugin-grid"></div>
      </section>
      <section id="v-account" class="view account-center">
        <div class="section-head"><div><div class="top-kicker">IDENTITY & PREFERENCES</div><h2 data-i18n="account.title">帳號與設定</h2><p data-i18n="account.subtitle">帳號安全、語言、外觀與目前身份。</p></div></div>
        <div class="account-overview">
          <article class="card identity-card-large"><span class="card-kicker">IDENTITY</span><h3 data-i18n="account.identity">目前身份</h3><div id="accountIdentitySummary" class="identity-summary">—</div></article>
          <article class="card"><span class="card-kicker">PREFERENCES</span><h3 data-i18n="account.preferences">介面偏好</h3><div class="field"><label data-i18n="top.language">語言</label><select id="accountLocaleSelect"></select></div><button id="accountThemeToggle" class="btn ghost" type="button">深色</button></article>
        </div>
        <div id="accountSecurityMount"></div>
      </section>
...
(function(){
'use strict';
var token='';var currentGroup='';var session=null;var groupsLoaded=false;var conversationCapabilities={recordViolation:true};var conversationPage=1,conversationPageSize=20,conversationTotalPages=1,conversationRequestSerial=0;var PORTAL_SIDEBAR_COLLAPSIBLE='v1';
var PORTAL_PLUGINS=Array.isArray(window.__QQAI_PORTAL_PLUGINS__)?window.__QQAI_PORTAL_PLUGINS__:[];var pluginCanManage=false;
var PORTAL_I18N=window.__QQAI_PORTAL_I18N__||{locales:[{id:'zh-TW',label:'繁體中文'},{id:'zh-CN',label:'简体中文'},{id:'en',label:'English'}],messages:{},defaultLocale:'zh-TW'};
var portalLocale='zh-TW';
var $=function(id){return document.getElementById(id)};
function normalizeUiLocale(value){var raw=String(value||'').toLowerCase();if(!raw)return'zh-TW';if(raw==='zh'||raw.indexOf('zh-tw')===0||raw.indexOf('zh-hk')===0||raw.indexOf('zh-hant')===0)return'zh-TW';if(raw.indexOf('zh-cn')===0||raw.indexOf('zh-sg')===0||raw.indexOf('zh-hans')===0)return'zh-CN';if(raw.indexOf('pt')===0)return'pt-BR';var exact=(PORTAL_I18N.locales||[]).find(function(x){return String(x.id).toLowerCase()===raw});if(exact)return exact.id;var base=raw.split('-')[0],match=(PORTAL_I18N.locales||[]).find(function(x){return String(x.id).toLowerCase().split('-')[0]===base});return match?match.id:'en'}
function t(key){var table=(PORTAL_I18N.messages||{})[portalLocale]||(PORTAL_I18N.messages||{}).en||{},fallback=(PORTAL_I18N.messages||{})['zh-TW']||{};return table[key]||fallback[key]||key}
function detectPortalLocale(){try{var saved=localStorage.getItem('qqai_locale');if(saved)return normalizeUiLocale(saved)}catch(e){}var langs=(navigator.languages&&navigator.languages.length?navigator.languages:[navigator.language||'zh-TW']);return normalizeUiLocale(langs[0])}
function fillLocaleSelect(select){if(!select)return;select.innerHTML=(PORTAL_I18N.locales||[]).map(function(x){return '<option value="'+esc(x.id)+'">'+esc(x.label)+'</option>'}).join('');select.value=portalLocale}
function pluginLocalizedCopy(plugin){var rows=plugin&&plugin.i18n||{},row=rows[portalLocale]||rows[normalizeUiLocale(portalLocale)]||rows.en||rows['zh-TW']||{};return{name:row.name||plugin.name||plugin.id,description:row.description||plugin.description||''}}
function pluginForViewClient(view){var name=String(view||'').toLowerCase();return PORTAL_PLUGINS.find(function(p){return((p.portal&&p.portal.views)||[]).includes(name)})||null}
function portalViewAvailable(view){var p=pluginForViewClient(view);if(p&&p.enabled===false)return false;var b=document.querySelector('#nav button[data-view="'+String(view||'').replace(/"/g,'')+'"]');return!!(b&&!b.hidden&&b.style.display!=='none')}
function pluginOpenView(plugin){if(!plugin||plugin.enabled===false)return'';return((plugin.portal&&plugin.portal.views)||[]).find(portalViewAvailable)||''}
function pluginUsesGroupContext(view){var p=pluginForViewClient(view);return!!(p&&p.portal&&p.portal.groupContext)}
async function ensurePluginContext(view){var needsGroup=pluginUsesGroupContext(view),select=$('groupSelect');if(select)select.classList.toggle('hidden',!needsGroup);if(!needsGroup)return true;if(groupsLoaded)return true;var loaded=await loadGroups();groupsLoaded=!!loaded;return groupsLoaded}
async function loadPlugins(){var r=await api('/plugins');if(!r.ok){toast(r.message||t('common.unavailable'));renderAccountIdentity();return false}PORTAL_PLUGINS=Array.isArray(r.plugins)?r.plugins:[];pluginCanManage=r.canManage===true;renderPluginCatalog();renderAccountIdentity();return true}
async function togglePlugin(pluginId,enabled){var r=await api('/plugins/'+encodeURIComponent(pluginId),'POST',{enabled:!!enabled});toast(r.message||t('common.unavailable'));if(r.ok){await loadPlugins();var current=String(location.hash||'').replace(/^#/,'');var owner=pluginForViewClient(current);if(owner&&owner.enabled===false)showView('plugins')}return r}
function renderPluginCatalog(){var dev=pluginCanManage||!!(session&&((session.permissions||{}).developer||session.role==='developer')),visible=PORTAL_PLUGINS.filter(function(p){return!(p.portal&&p.portal.developerOnly)||dev});function card(p,compact){var copy=pluginLocalizedCopy(p),enabled=p.enabled!==false,view=pluginOpenView(p),bridge=p.portal&&p.portal.legacyBridge,state='<span class="plugin-state '+(enabled?'on':'off')+'">'+esc(enabled?t('plugins.enabled'):t('plugins.disabled'))+'</span>',actions='';if(!compact){if(enabled&&view)actions+='<button type="button" class="btn primary" data-plugin-open="'+esc(p.id)+'">'+esc(t('plugins.open'))+'</button>';else if(enabled&&!view)actions+='<span class="plugin-availability">'+esc(t('common.unavailable'))+'</span>';if(dev)actions+='<button type="button" class="btn '+(enabled?'ghost':'primary')+' plugin-toggle" data-plugin-toggle="'+esc(p.id)+'" data-next-enabled="'+(enabled?'false':'true')+'">'+esc(enabled?t('plugins.disable'):t('plugins.enable'))+'</button>';}return '<article class="plugin-card'+(compact?' compact':'')+(enabled?'':' disabled')+'"><div class="plugin-icon">'+esc((p.portal&&p.portal.icon)||'◇')+'</div><div class="plugin-copy"><div class="plugin-card-head"><h3>'+esc(copy.name)+'</h3>'+state+(bridge?'<span class="plugin-badge">'+esc(t('plugins.bridge'))+'</span>':'')+'</div><p>'+esc(copy.description)+'</p></div>'+(actions?'<div class="plugin-actions">'+actions+'</div>':'')+'</article>'}var grid=$('pluginGrid');if(grid)grid.innerHTML=visible.map(function(p){return card(p,false)}).join('')||'<div class="empty">'+esc(t('common.unavailable'))+'</div>';var mini=$('overviewPluginGrid');if(mini)mini.innerHTML=visible.filter(function(p){return p.enabled!==false}).slice(0,4).map(function(p){return card(p,true)}).join('');document.querySelectorAll('[data-plugin-open]').forEach(function(b){b.onclick=function(){var p=PORTAL_PLUGINS.find(function(x){return x.id===b.dataset.pluginOpen}),view=pluginOpenView(p);if(view)showView(view)}});document.querySelectorAll('[data-plugin-toggle]').forEach(function(b){b.onclick=function(){togglePlugin(b.dataset.pluginToggle,b.dataset.nextEnabled==='true')}})}
function applyPortalI18n(){document.documentElement.lang=portalLocale;document.querySelectorAll('[data-i18n]').forEach(function(el){el.textContent=t(el.dataset.i18n)});fillLocaleSelect($('localeSelect'));fillLocaleSelect($('accountLocaleSelect'));updateThemeButtons();renderPluginCatalog();var active=document.querySelector('#nav button[data-view].active');if(active&&$('pageTitle'))$('pageTitle').textContent=portalTitle(active.dataset.view)}
function setPortalLocale(locale){portalLocale=normalizeUiLocale(locale);try{localStorage.setItem('qqai_locale',portalLocale)}catch(e){}applyPortalI18n()}
portalLocale=detectPortalLocale();
function activeTheme(){return document.documentElement.dataset.theme==='dark'?'dark':'light'}
function updateThemeButtons(){var dark=activeTheme()==='dark',label=dark?t('top.light'):t('top.dark');if($('loginThemeToggle'))$('loginThemeToggle').textContent=label;if($('themeToggle'))$('themeToggle').textContent=label;if($('accountThemeToggle'))$('accountThemeToggle').textContent=label}
...
function updatePortalAdvancedUi(){document.documentElement.classList.toggle('developer-mode',!!(session&&((session.permissions||{}).developer||session.role==='developer')));if($('advancedToggle'))$('advancedToggle').classList.add('hidden')}
...
function bindDashboardActions(){document.querySelectorAll('[data-open-view]').forEach(function(el){if(el.dataset.bound==='1')return;el.dataset.bound='1';el.addEventListener('click',function(){var view=this.dataset.openView,nav=document.querySelector('#nav button[data-view="'+view+'"]');if(!nav||nav.hidden||nav.style.display==='none'){toast(t('common.unavailable'));return}showView(view)})})}
var titles={overview:'首頁',plugins:'插件',account:'帳號與設定',health:'系統健康',tasks:'任務佇列',schedules:'排程提醒',moderation:'待確認操作',members:'群友列表',simulator:'事件模擬器',models:'AI 模型',quota:'額度管理',groups:'群組設定',memory:'AI 記憶',logs:'操作日誌',aidecisions:'AI 回覆紀錄',appeals:'我的申訴',appealreview:'申訴處理',violationhistory:'違規紀錄',conversations:'對話紀錄',platform:'功能權限',collaboration:'活動與投票',settingscenter:'更多設定',ruleviolations:'群規與複核',bilibili:'Bilibili'};
function portalTitle(name){return name==='overview'?t('nav.home'):name==='plugins'?t('nav.plugins'):name==='account'?t('nav.account'):name==='health'?t('nav.health'):(titles[name]||name)};
...
function portalRoleLabel(role){return({developer:'Developer / Root',owner:'群主',admin:'QQ 管理員',member:'群成員'})[String(role||'member')]||String(role||'群成員')}
function renderAccountIdentity(){var box=$('accountIdentitySummary'),hint=$('pluginManagementHint');var canManage=pluginCanManage||!!(session&&((session.permissions||{}).developer||session.role==='developer'||String(session.username||'').toLowerCase()==='admin'));if(hint){hint.classList.toggle('manager',!!canManage);hint.innerHTML=canManage?'<b>你有 Developer / Root 插件管理權限。</b><span>停用的插件按「啟用插件」；啟用後可直接開啟，或再按「停用插件」。狀態會儲存在 D1。</span>':'<b>你目前可使用已啟用的插件。</b><span>只有 Developer / Root 可以啟用或停用全域插件。</span>'}if(!box)return;if(!session){box.innerHTML='<div class="identity-empty">正在讀取身份資料…</div>';return}var perms=session.permissions||{},role=portalRoleLabel(session.role||'member'),username=String(session.username||session.account||session.qq||'未命名帳號'),qq=String(session.qq||'—'),group=String(session.group||session.groupName||session.groupId||'未選擇群組'),authority=canManage?'可管理插件':'一般使用權限';box.innerHTML='<div class="identity-primary"><span class="identity-avatar">'+esc(username.slice(0,1).toUpperCase()||'R')+'</span><div><b>'+esc(username)+'</b><small>QQ '+esc(qq)+'</small></div><span class="identity-badge '+(canManage?'manager':'')+'">'+esc(role)+'</span></div><div class="identity-meta-grid"><div><span>目前身份</span><b>'+esc(role)+'</b></div><div><span>插件權限</span><b>'+esc(authority)+'</b></div><div><span>目前群組</span><b>'+esc(group)+'</b></div><div><span>驗證來源</span><b>Server session</b></div></div>'}
function refreshAccountIdentity(){var box=$('accountIdentitySummary');if(box&&!session)box.innerHTML='<div class="identity-empty">正在讀取身份資料…</div>';return api('/me').then(function(me){if(me&&me.ok&&me.session){session=me.session;pluginCanManage=me.canManagePlugins===true||pluginCanManage}else if(box){box.innerHTML='<div class="identity-empty">身份資料讀取失敗，請重新整理。</div>'}renderAccountIdentity();return!!(me&&me.ok&&me.session)}).catch(function(){if(box)box.innerHTML='<div class="identity-empty">身份資料讀取失敗，請重新整理。</div>';return false})}
function applyRoleVisibility(){if(!session)return;var p=session.permissions||{},role=session.role||'member',dev=!!(p.developer||role==='developer'),management=!!(p.aiAdmin||p.groupOps||p.nativeAdmin||dev||['admin','owner'].includes(role));var memberViews=['overview','plugins','account','collaboration','schedules','memory','appeals','violationhistory','settingscenter','models'];var advancedViews=['maintenance','health','tasks','simulator','quota','platform','logs'];document.querySelectorAll('#nav button[data-view]').forEach(function(b){var v=b.dataset.view,show=true;if(role==='member'&&!memberViews.includes(v))show=false;if(['groups','moderation','members','ruleviolations','bilibili','aidecisions','conversations'].includes(v)&&!management)show=false;if(v==='appealreview'&&!(p.appealReviewer||p.nativeAdmin||dev||['admin','owner'].includes(role)))show=false;if(advancedViews.includes(v)&&!dev)show=false;b.style.display=show?'':'none';b.hidden=!show});updatePortalAdvancedUi();syncDashboardActions();refreshSidebarGroupVisibility()}
...
function ensureR3Views(){
  ...
  if(!$('v-platform')){var b=document.createElement('button');b.dataset.view='platform';b.textContent='功能权限中心';b.hidden=true;$('nav').appendChild(b);b.onclick=function(){showView('platform')};var v=document.createElement('section');v.id='v-platform';v.className='view';v.innerHTML='<div class="section-head"><div><h2>功能能力目录</h2><p>这里展示名称、类别与声明的实现方式。目前这些目录项目没有全部接入运行时控制，因此本页只读，不会开关机器人的实际行为。</p></div><button id="pfReload" class="btn">重新加载</button></div><div class="card"><div class="row"><input id="pfSearch" class="grow" placeholder="搜索功能名称、ID、类别"><button id="pfGo" class="btn primary">搜索</button></div><div id="pfSummary" class="notice">尚未加载</div></div><div id="pfList" class="list" style="margin-top:16px"></div>';document.querySelector('.content').appendChild(v);$('pfReload').onclick=loadPlatformFeatures;$('pfGo').onclick=loadPlatformFeatures;$('pfSearch').onkeydown=function(e){if(e.key==='Enter')loadPlatformFeatures()}}
...
function refreshSidebarGroupVisibility(){renderPluginCatalog()}
...
function showLogin(){location.replace('/login')}
function showApp(){$('login').classList.add('hidden');$('app').classList.remove('hidden');closeMobileSidebar();syncResponsivePortal()}
async function loadPlatformFeatures(){var q=$('pfSearch')?$('pfSearch').value:'';var r=await api('/platform/features?q='+encodeURIComponent(q));if(!r.ok){$('pfList').innerHTML='<div class=\'empty\'>'+esc(r.message)+'</div>';return}$('pfSummary').textContent='能力目录 '+r.total+' 项；当前显示 '+r.visible+' 项。只读目录不会更改实际执行状态。';$('pfList').innerHTML='';(r.features||[]).forEach(function(f){var d=document.createElement('div');d.className='item';d.innerHTML='<div class=\'item-head\'><div><div class=\'item-title\'>'+esc(f.id)+'｜'+esc(f.name)+'</div><div class=\'item-meta\'>类别：'+esc(f.category)+'｜声明实现方式：'+esc(f.mode)+'｜最低权限：'+esc(portalRoleLabel(f.minRole))+'｜能力目录（唯读）</div></div></div>';$('pfList').appendChild(d)});if(!$('pfList').children.length)$('pfList').innerHTML='<div class=\'empty\'>没有符合当前权限或搜索条件的功能</div>'}
async function showView(name){var owner=pluginForViewClient(name),view=$('v-'+name),navButton=document.querySelector('#nav button[data-view="'+name+'"]');if((owner&&owner.enabled===false)||!view||!navButton||navButton.hidden||navButton.style.display==='none'){toast(t('common.unavailable'));return}if(!(await ensurePluginContext(name)))return;document.querySelectorAll('.view').forEach(function(v){v.classList.remove('active')});document.querySelectorAll('#nav button[data-view]').forEach(function(b){b.classList.toggle('active',b.dataset.view===name)});expandSidebarGroupForView(name);refreshSidebarGroupVisibility();view.classList.add('active');var pluginOwned=!!owner;document.documentElement.classList.toggle('plugin-workspace',pluginOwned);if($('pluginBack')){$('pluginBack').classList.toggle('hidden',!pluginOwned);$('pluginBack').onclick=function(){showView('plugins')}}if($('pageContext'))$('pageContext').textContent=pluginOwned?pluginLocalizedCopy(owner).name:'CORE WORKSPACE';$('pageTitle').textContent=portalTitle(name);try{history.replaceState(null,'','#'+name)}catch(e){}closeMobileSidebar();if(name==='overview')refreshOverview();if(name==='account')await refreshAccountIdentity();if(name==='maintenance'){loadOperations();['opsRules','opsAppeal'].forEach(opsLoadWorkspace)}if(name==='health'){loadHealth('quick');loadModelCheckCandidates();loadOperations()}if(name==='tasks'){loadTasks();loadOperations();opsLoadTasks()}if(name==='collaboration'){loadOperations();opsLoadWorkspace('opsCollab')}if(name==='schedules'){loadSchedules();loadOperations()}if(name==='moderation')loadProposals();if(name==='models')loadModels();if(name==='quota')loadQuota();if(name==='groups')loadGroupSettings();if(name==='memory'){loadMemory();loadOperations();opsLoadWorkspace('opsKnowledge')}if(name==='logs')loadLogs();if(name==='aidecisions')loadAiDecisions();else if(name==='appeals')loadAppeals();else if(name==='appealreview')loadAppealReviews();if(name==='ruleviolations')loadRuleViolations();if(name==='violationhistory')loadViolationHistory();if(name==='settingscenter'){loadSettingsCenter();loadOperations()}if(name==='bilibili')loadBilibili();if(name==='platform')loadPlatformFeatures();if(name==='conversations')loadConversations()}
...
async function selectGroup(id){if(!id){currentGroup='';setNativeAdminVisibility(false);refreshOverview();return}var r=await api('/select-group','POST',{groupId:id});if(!r.ok){toast(r.message);return}currentGroup=id;session=r.session;$('identity').innerHTML='<b>'+esc(session.username||'已登入帳號')+'</b><br><span style="color:#98a2b7">'+esc(portalRoleLabel(session.role||'member'))+'</span>';renderAccountIdentity();await refreshCapabilities();applyR3RoleVisibility();toast('已切换群组');refreshOverview()}
function ensureAccountSecurityPanel(){var view=$('v-account')||$('v-settingscenter');if(!view||$('accountSecurityPanel'))return;var panel=document.createElement('div');panel.id='accountSecurityPanel';panel.className='card';panel.style.marginBottom='16px';panel.innerHTML='<div class="section-head" style="margin-bottom:12px"><div><h3>登录与双因数验证</h3><p>验证码登录永远保留；密码为可选。启用 2FA 后，密码登录还需要验证器动态码、单次备用码或 QQ 私信验证码。</p></div><button id="authSecurityReload" class="btn">刷新状态</button></div><div id="authSecurityStatus" class="notice">尚未加载。</div><div class="security-grid" style="margin-top:14px"><div class="card"><h3>设置或修改密码</h3><div class="field"><label>当前密码（首次设置可留空）</label><input id="authCurrentPassword" type="password" autocomplete="current-password"></div><div class="field"><label>新密码（至少 10 个字符）</label><input id="authNewPassword" type="password" maxlength="128" autocomplete="new-password"></div><div class="field"><label>确认新密码</label><input id="authConfirmPassword" type="password" maxlength="128" autocomplete="new-password"></div><div class="field"><label>QQ 验证码（登录超过 15 分钟或忘记当前密码时使用）</label><input id="authVerificationCode" inputmode="numeric" maxlength="6" autocomplete="one-time-code"></div><div class="row"><button id="authSendCode" class="btn ghost">发送 QQ 验证码</button><button id="authSavePassword" class="btn primary">保存密码</button></div></div><div class="card"><h3>双因数验证（TOTP）</h3><div class="field"><label>当前密码</label><input id="auth2faPassword" type="password" autocomplete="current-password"></div><div class="row"><button id="auth2faSetup" class="btn primary">开始设置 2FA</button><button id="auth2faDisable" class="btn danger">关闭 2FA</button></div><div id="auth2faSetupArea" class="hidden"><div class="field"><label>验证器密钥</label><input id="auth2faSecret" readonly></div><div class="field"><label>otpauth URI</label><textarea id="auth2faUri" readonly></textarea></div><div class="field"><label>验证器当前六位动态码</label><input id="auth2faCode" inputmode="numeric" maxlength="6" autocomplete="one-time-code"></div><div class="row"><button id="auth2faEnable" class="btn primary">验证并启用</button><button id="authBackupRegenerate" class="btn">重新生成备用码</button></div></div></div></div><div id="authBackupCodes" class="notice hidden backup-codes"></div>';var mount=$('accountSecurityMount');if(mount)mount.appendChild(panel);else{var anchor=$('scMessage');view.insertBefore(panel,anchor||view.firstChild)}$('authSecurityReload').onclick=loadAccountSecurity;$('authSendCode').onclick=sendAccountSecurityCode;$('authSavePassword').onclick=saveAccountPassword;$('auth2faSetup').onclick=setupAccount2fa;$('auth2faEnable').onclick=enableAccount2fa;$('auth2faDisable').onclick=disableAccount2fa;$('authBackupRegenerate').onclick=regenerateBackupCodes}
async function loadAccountSecurity(){if(!$('authSecurityStatus'))return;var r=await api('/security/auth-state');if(!r.ok){$('authSecurityStatus').textContent=r.message||'读取失败';return}$('authSecurityStatus').textContent='密码：'+(r.passwordSet?'已设置':'未设置')+(r.adminPasswordManaged?'（由 Cloudflare PORTAL_ADMIN_PASSWORD Secret 管理）':'')+(r.adminCredentialsMisconfigured?'；管理員帳號環境變數設定不完整':'')+'；双因数验证：'+(r.twoFactorEnabled?'已启用':'未启用')+(r.twoFactorEnabled?'；剩余备用码 '+Number(r.backupCodesRemaining||0)+' 组':'')+(r.encryptionReady?'':'；尚未設定 2FA 加密 Secret，暫時不能啟用 2FA');$('auth2faSetup').disabled=!r.passwordSet||!r.encryptionReady||r.twoFactorEnabled;$('auth2faDisable').disabled=!r.twoFactorEnabled;$('authBackupRegenerate').disabled=!r.twoFactorEnabled;$('auth2faSetupArea').classList.toggle('hidden',!r.twoFactorEnabled);if(r.adminPasswordManaged){['authCurrentPassword','authNewPassword','authConfirmPassword','authVerificationCode','authSendCode','authSavePassword'].forEach(function(id){if($(id))$(id).disabled=true})}}
function ensurePortalDeveloperPanel(){if(!session||String(session.username||'').toLowerCase()!=='admin'||$('portalDeveloperPanel'))return;var mount=$('accountSecurityMount');if(!mount)return;var panel=document.createElement('div');panel.id='portalDeveloperPanel';panel.className='card';panel.style.marginTop='16px';panel.innerHTML='<div class="section-head"><div><h3>Developer QQ 管理</h3><p>只有系統管理員可以變更此清單。Cloudflare 中設定的 Developer／Root QQ 會列在唯讀清單。</p></div><button id="portalDeveloperReload" class="btn">重新載入</button></div><div class="field"><label for="portalDeveloperIds">額外 Developer QQ（5–12 位數字，每行一個，最多 50 個）</label><textarea id="portalDeveloperIds" rows="7" spellcheck="false" placeholder="例如：123456789"></textarea></div><div id="portalDeveloperConfigured" class="notice">Cloudflare 設定的 Developer／Root QQ：讀取中…</div><div class="row"><button id="portalDeveloperSave" class="btn primary">儲存 Developer QQ</button></div><div id="portalDeveloperMessage" class="notice">只有管理員帳號可以加入或移除 Developer QQ。</div>';mount.appendChild(panel);$('portalDeveloperReload').onclick=loadPortalDeveloperIds;$('portalDeveloperSave').onclick=savePortalDeveloperIds}
async function loadPortalDeveloperIds(){if(!$('portalDeveloperIds'))return;var r=await api('/system/developers');if(!r.ok){$('portalDeveloperMessage').textContent=r.message||'Developer QQ 清單讀取失敗。';return}$('portalDeveloperIds').value=(r.managedIds||[]).join('\n');$('portalDeveloperConfigured').textContent='Cloudflare 設定的 Developer／Root QQ（唯讀）：'+((r.configuredIds||[]).join('、')||'尚未設定');$('portalDeveloperMessage').textContent='清單已載入；儲存後會套用於 Portal 與機器人權限判定。'}
async function savePortalDeveloperIds(){var ids=String($('portalDeveloperIds').value||'').split(/[\n,;，；]+/).map(function(id){return id.trim()}).filter(Boolean);var r=await api('/system/developers','POST',{ids:ids});$('portalDeveloperMessage').textContent=r.message||'Developer QQ 更新失敗。';if(r.ok){$('portalDeveloperIds').value=(r.managedIds||[]).join('\n');toast(r.message||'Developer QQ 清單已更新。')}}
...
async function boot(attempt){attempt=Number(attempt||0);migratePortalMaintenanceV140();ensureR3Views();ensureAccountSecurityPanel();organizeSidebarNavigation();bindDashboardActions();applyPortalI18n();var me=await api('/me');if(!me.ok){if(me.retryable||me.code==='SESSION_STORAGE_UNAVAILABLE'){setNativeAdminVisibility(false);$('loginNotice').textContent=me.message||'登录会话资料库暂时不可用，正在重试…';if(attempt<3){setTimeout(function(){boot(attempt+1)},500*(attempt+1));return}showLogin();return}setNativeAdminVisibility(false);showLogin();return}showApp();session=me.session;ensurePortalDeveloperPanel();loadPortalDeveloperIds();loadAccountSecurity();$('identity').innerHTML='<b>'+esc(session.username||session.qq||'admin')+'</b><br><span>'+esc(portalRoleLabel(session.role||'member'))+'</span>';renderAccountIdentity();await loadPlugins();renderAccountIdentity();applyR3RoleVisibility();applyPortalI18n();var hash=String(location.hash||'').replace(/^#/,'');if(hash&&$('v-'+hash)){var b=document.querySelector('#nav button[data-view="'+hash+'"]');if(b&&!b.hidden&&b.style.display!=='none')await showView(hash);else await showView('overview')}else await showView('overview')}
...
async function refreshOverview(){var dev=!!(session&&((session.permissions||{}).developer||session.role==='developer')),visible=PORTAL_PLUGINS.filter(function(p){return!(p.portal&&p.portal.developerOnly)||dev}),enabled=visible.filter(function(p){return p.enabled!==false});if($('coreAccountName'))$('coreAccountName').textContent=session&&(session.username||session.qq)||'—';if($('coreRoleLabel'))$('coreRoleLabel').textContent=session?portalRoleLabel(session.role||'member'):'—';if($('coreEnabledPlugins'))$('coreEnabledPlugins').textContent=enabled.length;if($('coreTotalPlugins'))$('coreTotalPlugins').textContent=visible.length;if($('coreAuthority'))$('coreAuthority').textContent=dev?'Developer / Root':'server-side';renderPluginCatalog()}
...
var loginProfiles={developer:{title:'開發者入口',description:'平台核心維護、部署與安全管理。建議使用密碼與第二因素登入；實際權限仍由伺服器判定。'},instance_admin:{title:'實例管理員入口',description:'管理自己的部署、Cloudflare 資源與插件。入口不會授予管理權限，登入後只顯示帳號實際擁有的能力。'},ai_user:{title:'AI 使用者入口',description:'適合直接使用 AI、個人設定與可用額度的帳號。可使用訊息驗證碼或已設定的密碼登入。'},community_member:{title:'社群成員入口',description:'適合只參與已啟用服務之社群的人。登入後只會看到你實際被授權的功能。'}};
function setLoginProfile(profile){if(!loginProfiles[profile])profile='ai_user';document.querySelectorAll('[data-login-profile]').forEach(function(btn){btn.classList.toggle('active',btn.dataset.loginProfile===profile)});$('loginProfileTitle').textContent=loginProfiles[profile].title;$('loginProfileDescription').textContent=loginProfiles[profile].description;try{localStorage.setItem('ai_control_login_profile',profile)}catch(e){}}
function publicAuthMessage(r,fallback){var code=String(r&&r.code||'');if(code==='VERIFICATION_DELIVERY_FAILED'&&r&&r.message)return r.message;var map={VERIFICATION_DELIVERY_FAILED:'驗證碼目前無法送達，請確認訊息連線狀態或稍後再試。',AUTH_STORAGE_UNAVAILABLE:'登入服務暫時無法使用，請稍後再試。',SESSION_STORAGE_UNAVAILABLE:'驗證成功，但登入工作階段暫時無法建立，請稍後再試。',PASSWORD_NOT_SET:'此帳號尚未設定密碼，請先使用訊息驗證碼登入。',PASSWORD_RECORD_INVALID:'密碼資料需要更新，請使用安全／復原登入重新設定。',PASSWORD_INVALID:'帳號 ID 或密碼錯誤。',PASSWORD_LOGIN_LOCKED:'登入嘗試次數過多，請稍後再試或改用訊息驗證碼。',TWO_FACTOR_REQUIRED:'密碼正確，請再完成第二因素驗證。'};return map[code]||(r&&r.ok?fallback:(fallback||'操作失敗，請稍後再試。'))}
function setLoginMethod(method){var password=method==='password';$('loginCodePane').classList.toggle('hidden',password);$('loginPasswordPane').classList.toggle('hidden',!password);$('loginMethodCode').classList.toggle('active',!password);$('loginMethodPassword').classList.toggle('active',password);$('loginMethodNote').textContent=password?'輸入已設定的密碼；若帳號已啟用第二因素，還需要動態碼、備用碼或訊息驗證碼。':'驗證碼會透過你已連接的訊息服務傳送。';$('loginNotice').textContent=password?'密碼登入使用既有安全驗證與鎖定機制。':'取得驗證碼不會改變帳號權限，登入後仍由伺服器決定可使用功能。'}
async function requestLoginCode(){var qq=String($('loginQq').value||'').replace(/\D/g,'');if(!/^\d{5,12}$/.test(qq)){$('loginNotice').textContent='請輸入正確的帳號 ID。';return}try{localStorage.setItem('qqai_last_login_qq',qq)}catch(e){}var b=$('sendCode');b.disabled=true;b.textContent='傳送中…';$('loginNotice').textContent='正在傳送驗證碼…';var r=await raw('/api/auth/request-code','POST',{qq:qq});$('loginNotice').textContent=publicAuthMessage(r,r.ok?'驗證碼已送出，請在有效時間內輸入。':'驗證碼傳送失敗。');b.disabled=false;b.textContent=r.ok?'重新傳送驗證碼':'傳送驗證碼'}
async function verifyLoginCode(){var qq=String($('loginQq').value||'').replace(/\D/g,''),code=String($('loginCode').value||'').replace(/\D/g,'');if(!/^\d{5,12}$/.test(qq)||!/^\d{6}$/.test(code)){$('loginNotice').textContent='請輸入正確的帳號 ID 與六位驗證碼。';return}var b=$('verifyCode');b.disabled=true;b.textContent='驗證中…';var r=await raw('/api/auth/verify-code','POST',{qq:qq,code:code,remember:!$('rememberLogin')||$('rememberLogin').checked});$('loginNotice').textContent=publicAuthMessage(r,r.ok?'登入成功，正在進入控制中心。':'驗證失敗。');b.disabled=false;b.textContent='使用驗證碼登入';if(r.ok){await boot()}}
async function verifyPasswordLogin(){var username=String($('loginQq').value||'').trim(),password=$('loginPassword').value;if(!username||!password){$('loginNotice').textContent='請輸入帳號與密碼。';return}var b=$('verifyPassword');b.disabled=true;b.textContent='驗證中…';var factorVisible=!$('loginFactorWrap').classList.contains('hidden');var r=await raw('/api/auth/login-password','POST',{username:username,password:password,remember:!$('rememberLogin')||$('rememberLogin').checked,factorType:factorVisible?$('loginFactorType').value:'',factorCode:factorVisible?$('loginFactorCode').value:''});b.disabled=false;b.textContent='使用密碼登入';$('loginNotice').textContent=publicAuthMessage(r,r.ok?'登入成功，正在進入控制中心。':'登入失敗。');if(r.code==='PASSWORD_RECORD_INVALID'&&$('loginPasswordReset'))$('loginPasswordReset').open=true;if(r.requiresTwoFactor||r.code==='TWO_FACTOR_REQUIRED'){$('loginFactorWrap').classList.remove('hidden');$('loginFactorCode').focus();return}if(r.ok){await boot()}}
async function requestPasswordFactorCode(){var username=String($('loginQq').value||'').trim(),password=$('loginPassword').value;if(!username||!password){toast('請先輸入帳號與密碼');return}var r=await raw('/api/auth/request-login-factor','POST',{username:username,password:password});$('loginNotice').textContent=publicAuthMessage(r,r.ok?'第二因素驗證碼已送出。':'驗證碼傳送失敗。');if(r.ok){$('loginFactorType').value='qq_code';$('loginFactorWrap').classList.remove('hidden')}}
async function requestPasswordResetCode(){var qq=String($('loginQq').value||'').replace(/\D/g,'');if(!/^\d{5,12}$/.test(qq)){$('loginNotice').textContent='請先輸入正確的帳號 ID。';return}var r=await raw('/api/auth/request-code','POST',{qq:qq});$('loginNotice').textContent=publicAuthMessage(r,r.ok?'復原驗證碼已送出。':'復原驗證碼傳送失敗。');if(r.ok&&$('loginPasswordReset'))$('loginPasswordReset').open=true}
async function resetPasswordLogin(){var qq=String($('loginQq').value||'').replace(/\D/g,''),next=$('loginResetPassword').value,confirm=$('loginResetConfirm').value,code=String($('loginResetCode').value||'').replace(/\D/g,'');if(!/^\d{5,12}$/.test(qq)){$('loginNotice').textContent='請輸入正確的帳號 ID。';return}if(next!==confirm){$('loginNotice').textContent='兩次輸入的新密碼不一致。';return}if(next.length<10||!/^\d{6}$/.test(code)){$('loginNotice').textContent='請輸入至少 10 個字元的新密碼與六位驗證碼。';return}var b=$('passwordResetSubmit');b.disabled=true;b.textContent='重設中…';var r=await raw('/api/auth/reset-password','POST',{qq:qq,code:code,newPassword:next});b.disabled=false;b.textContent='重設密碼';$('loginNotice').textContent=publicAuthMessage(r,r.ok?'密碼已重設，可以使用新密碼登入。':'密碼重設失敗。');if(r.ok){$('loginPassword').value=next;$('loginResetPassword').value='';$('loginResetConfirm').value='';$('loginResetCode').value='';if($('loginPasswordReset'))$('loginPasswordReset').open=false}}
try{var lastLoginQq=localStorage.getItem('qqai_last_login_qq');if(lastLoginQq)$('loginQq').value=lastLoginQq;var lastProfile=localStorage.getItem('ai_control_login_profile');if(lastProfile&&loginProfiles[lastProfile])setLoginProfile(lastProfile);else setLoginProfile('developer')}catch(e){setLoginProfile('developer')}document.querySelectorAll('[data-login-profile]').forEach(function(btn){btn.addEventListener('click',function(){setLoginProfile(this.dataset.loginProfile)})});$('loginMethodCode').addEventListener('click',function(){setLoginMethod('code')});$('loginMethodPassword').addEventListener('click',function(){setLoginMethod('password')});$('sendCode').addEventListener('click',requestLoginCode);$('verifyCode').addEventListener('click',verifyLoginCode);$('verifyPassword').addEventListener('click',verifyPasswordLogin);$('passwordSendFactorCode').addEventListener('click',requestPasswordFactorCode);$('passwordResetSendCode').addEventListener('click',requestPasswordResetCode);$('passwordResetSubmit').addEventListener('click',resetPasswordLogin);$('loginQq').addEventListener('keydown',function(e){if(e.key==='Enter'){if($('loginPasswordPane').classList.contains('hidden'))requestLoginCode();else verifyPasswordLogin()}});$('loginCode').addEventListener('keydown',function(e){if(e.key==='Enter')verifyLoginCode()});$('loginPassword').addEventListener('keydown',function(e){if(e.key==='Enter')verifyPasswordLogin()});$('loginFactorCode').addEventListener('keydown',function(e){if(e.key==='Enter')verifyPasswordLogin()});$('loginThemeToggle').addEventListener('click',toggleTheme);$('themeToggle').addEventListener('click',toggleTheme);if($('accountThemeToggle'))$('accountThemeToggle').addEventListener('click',toggleTheme);if($('localeSelect'))$('localeSelect').addEventListener('change',function(){setPortalLocale(this.value)});if($('accountLocaleSelect'))$('accountLocaleSelect').addEventListener('change',function(){setPortalLocale(this.value)});applyPortalI18n();
$('logout').onclick=async function(){await raw('/api/auth/logout','POST',{});location.reload()};if($('advancedToggle'))$('advancedToggle').onclick=function(){setPortalAdvanced(!portalAdvancedEnabled())};bindDashboardActions();$('menu').onclick=toggleMobileSidebar;if($('sidebarBackdrop'))$('sidebarBackdrop').onclick=closeMobileSidebar;document.addEventListener('keydown',function(e){if(e.key==='Escape')closeMobileSidebar()});window.addEventListener('resize',syncResponsivePortal,{passive:true});syncResponsivePortal();$('refresh').onclick=function(){var active=document.querySelector('#nav button[data-view].active');showView(active?active.dataset.view:'overview')};if($('groupSelect'))$('groupSelect').onchange=function(){selectGroup(this.value)};document.querySelectorAll('#nav button[data-view]').forEach(function(b){b.onclick=function(){showView(b.dataset.view)}});
