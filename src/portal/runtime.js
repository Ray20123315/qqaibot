...
<section id="login" class="login hidden">
  <div class="login-card">
    ...
    <div class="login-main">
      ...
      <div class="field"><label for="loginQq">帳號 ID</label><input id="loginQq" inputmode="numeric" autocomplete="username" placeholder="輸入你的帳號識別碼"></div>
...
async function verifyPasswordLogin(){var qq=String($('loginQq').value||'').replace(/\D/g,''),password=$('loginPassword').value;if(!/^\d{5,12}$/.test(qq)||!password){$('loginNotice').textContent='請輸入正確的帳號 ID 與密碼。';return}var b=$('verifyPassword');b.disabled=true;b.textContent='驗證中…';var factorVisible=!$('loginFactorWrap').classList.contains('hidden');var r=await raw('/api/auth/login-password','POST',{qq:qq,password:password,remember:!$('rememberLogin')||$('rememberLogin').checked,factorType:factorVisible?$('loginFactorType').value:'',factorCode:factorVisible?$('loginFactorCode').value:''});b.disabled=false;b.textContent='使用密碼登入';$('loginNotice').textContent=publicAuthMessage(r,r.ok?'登入成功，正在進入控制中心。':'登入失敗。');if(r.code==='PASSWORD_RECORD_INVALID'&&$('loginPasswordReset'))$('loginPasswordReset').open=true;if(r.requiresTwoFactor||r.code==='TWO_FACTOR_REQUIRED'){$('loginFactorWrap').classList.remove('hidden');$('loginFactorCode').focus();return}if(r.ok){await boot()}}
async function requestPasswordFactorCode(){var qq=String($('loginQq').value||'').replace(/\D/g,'');if(!/^\d{5,12}$/.test(qq)){toast('請先輸入正確的帳號 ID');return}var r=await raw('/api/auth/request-code','POST',{qq:qq});$('loginNotice').textContent=publicAuthMessage(r,r.ok?'第二因素驗證碼已送出。':'驗證碼傳送失敗。');if(r.ok){$('loginFactorType').value='qq_code';$('loginFactorWrap').classList.remove('hidden')}}
