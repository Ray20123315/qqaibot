...
import { getPortalMaintenanceState, portalMaintenanceViewerCanBypass, setPortalMaintenanceState } from "./maintenance.js";
...
async function handlePortalApi(request, env, url) {
  ...
  if (path === "/maintenance") {
    if (!portalMaintenanceViewerCanBypass(env, session)) return jsonResponse({ ok: false, code: "MAINTENANCE_ADMIN_REQUIRED", message: "只有开发者或系统管理员可以管理系统维护模式。" }, 403);
    if (request.method === "GET") return jsonResponse({ ok: true, state: await getPortalMaintenanceState(env) });
    if (request.method !== "POST") return jsonResponse({ ok: false, code: "METHOD_NOT_ALLOWED", message: "此接口只允许 GET 或 POST。" }, 405, { Allow: "GET, POST" });
    try {
      const state = await setPortalMaintenanceState(env, body, session.systemAdmin ? "system-admin" : session.qq);
      await writeSystemAudit(env, { type: "portal_maintenance", actorId: session.systemAdmin ? "system-admin" : session.qq, action: state.enabled ? "enable" : "disable", until: state.until || 0 }).catch(() => {});
      return jsonResponse({ ok: true, state, message: state.enabled ? "系统维护模式已启用，普通用户与 Portal 后端请求将显示维护状态。" : "系统维护模式已关闭，普通用户可以重新进入控制台。" });
    } catch (error) {
      const invalid = String(error?.code || error?.message || "") === "MAINTENANCE_UNTIL_INVALID";
      return jsonResponse({ ok: false, code: invalid ? "MAINTENANCE_UNTIL_INVALID" : "MAINTENANCE_SAVE_FAILED", message: invalid ? "预计恢复时间必须晚于现在。" : "维护状态无法保存，请稍后重试。" }, invalid ? 400 : 503);
    }
  }
...
说明：${item.result}`).catch(() => null);
  ...
  if (path === "/simulator") return jsonResponse({ ok: false, code: "FEATURE_REMOVED", message: "事件模拟器已删除。" }, 410);
      ...
      <section id="v-overview" class="view active">
        ...
        <div class="grid overview-grid">
          ...
          <div class="card span-5"><div class="section-head compact"><div><h3>常用功能</h3><p>从这里开始，不需要理解后台术语。</p></div></div><div class="action-grid">
            ...
            <button class="action-card" data-open-view="member-data"><span class="action-icon">友</span><span><b>成员资料</b><small>群友名册、详细资料与清人建议</small></span></button>
...
var titles={overview:'首页',health:'系统诊断',tasks:'任务队列',schedules:'排程提醒',moderation:'待确认操作',members:'群友列表',simulator:'事件模拟器',models:'AI 模型',quota:'额度管理',groups:'群组设置',memory:'AI 记忆',logs:'操作日志',aidecisions:'AI 回复记录',appeals:'我的申诉',appealreview:'申诉处理',violationhistory:'违规记录',conversations:'对话记录',platform:'平台功能目录',collaboration:'活动与投票',settingscenter:'更多设置',ruleviolations:'群规与复核',bilibili:'B站监控'};
...
function ensureR3Views(){
  ...
  if(!$('v-maintenance').dataset.ready){$('v-maintenance').dataset.ready='1';$('v-maintenance').innerHTML='<div class="section-head"><div><h2>系统维护</h2><p>启用后，普通用户会看到维护页面，Portal 后端请求会返回 503；开发者与系统管理员登录后仍可进入维护。</p></div><button id="maintenanceReload" class="btn">重新加载</button></div><div class="grid"><div class="card span-7"><h3>维护状态</h3><label class="switch"><input id="portalMaintenanceEnabled" type="checkbox">启用系统维护</label><div class="field"><label>维护页标题</label><input id="portalMaintenanceTitle" maxlength="120" value="系统维护中"></div><div class="field"><label>维护说明</label><textarea id="portalMaintenanceMessage" maxlength="1200" placeholder="例如：正在升级服务，预计稍后恢复。"></textarea></div><div class="field"><label>预计恢复时间（可留空）</label><input id="portalMaintenanceUntil" type="datetime-local"></div><button id="portalMaintenanceSave" class="btn primary">保存维护状态</button><div id="portalMaintenanceStatus" class="notice" style="margin-top:12px">尚未读取。</div></div><div class="card span-5"><h3>维护期间行为</h3><p class="muted">普通用户访问控制台时只会看到维护画面；Portal API 会返回 503，不会进入后端业务处理。机器人 OneBot、Codex WebSocket、健康检查与公开 V3 状态不会被这项 Portal 维护模式中断。</p><p class="muted">开发者与系统管理员可从维护页的“开发者入口”登录；完成认证后会绕过维护拦截，以便检查与恢复系统。</p></div></div>';$('maintenanceReload').onclick=loadPortalMaintenance;$('portalMaintenanceSave').onclick=savePortalMaintenance}
...
async function loadPortalMaintenance(){if(!session||!(session.systemAdmin||(session.permissions&&session.permissions.developer)))return;var r=await api('/maintenance');var status=$('portalMaintenanceStatus');if(!r.ok){if(status)status.textContent=r.message||'维护状态读取失败';return}var s=r.state||{};if($('portalMaintenanceEnabled'))$('portalMaintenanceEnabled').checked=!!s.enabled;if($('portalMaintenanceTitle'))$('portalMaintenanceTitle').value=s.title||'系统维护中';if($('portalMaintenanceMessage'))$('portalMaintenanceMessage').value=s.message||'';if($('portalMaintenanceUntil')){$('portalMaintenanceUntil').value=s.until?new Date(Number(s.until)-new Date().getTimezoneOffset()*60000).toISOString().slice(0,16):''}if(status)status.textContent=s.enabled?'系统维护已启用｜普通用户与 Portal API 已被拦截。':'系统维护未启用。'}
async function savePortalMaintenance(){if(!session||!(session.systemAdmin||(session.permissions&&session.permissions.developer)))return;var untilValue=$('portalMaintenanceUntil')&&$('portalMaintenanceUntil').value;var payload={enabled:!!($('portalMaintenanceEnabled')&&$('portalMaintenanceEnabled').checked),title:String($('portalMaintenanceTitle')&&$('portalMaintenanceTitle').value||'系统维护中'),message:String($('portalMaintenanceMessage')&&$('portalMaintenanceMessage').value||''),until:untilValue?new Date(untilValue).getTime():0};var button=$('portalMaintenanceSave');if(button)button.disabled=true;var r=await api('/maintenance','POST',payload);if(button)button.disabled=false;toast(r.message||(r.ok?'维护状态已保存':'保存失败'));if(r.ok)loadPortalMaintenance()}
...
$('reloadModels').onclick=loadModels;$('reloadQuota').onclick=loadQuota;$('providerRouteTask').onchange=function(){renderProviderRouteSummary(window.__qqaiProviderState||{})};$('saveProviderAccount').onclick=async function(){var id=String($('providerAccountId').value||'').trim();if(!id){toast('請先填 Provider 帳號 ID');return}var payload={action:'upsert',id:id,provider:$('providerType').value,label:$('providerLabel').value,model:$('providerModel').value,tasks:String($('providerTasks').value||'').split(',').map(function(x){return x.trim()}).filter(Boolean),endpoint:$('providerEndpoint').value,accountId:$('providerCloudflareAccountId').value,gatewayId:$('providerGatewayId').value,secretEnv:$('providerSecretEnv').value,quota:{dailyMoney:providerMaybeNumber('providerDailyMoney'),monthlyMoney:providerMaybeNumber('providerMonthlyMoney'),dailyInputTokens:providerMaybeNumber('providerDailyInput'),dailyOutputTokens:providerMaybeNumber('providerDailyOutput'),remainingMoneyReported:providerMaybeNumber('providerRemainingMoney'),remainingTokensReported:providerMaybeNumber('providerRemainingTokens')}};if($('providerSecret').value)payload.secret=$('providerSecret').value;var r=await api('/root/ai-providers','POST',payload);toast(r.message||'保存完成');if(r.ok){$('providerSecret').value='';loadQuota()}};$('saveProviderRoute').onclick=async function(){var ids=String($('providerRouteIds').value||'').split(',').map(function(x){return x.trim()}).filter(Boolean);var r=await api('/root/ai-providers','POST',{action:'route',task:$('providerRouteTask').value,accountIds:ids});toast(r.message||'路由已保存');if(r.ok)loadQuota()};$('saveQuota').onclick=async function(){var r=await api('/root/quotas','POST',{globalDailyCny:$('globalQuota').value,groupDailyCny:$('groupQuota').value});toast(r.message);if(r.ok)loadQuota()};
