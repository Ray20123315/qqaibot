...
function packageManagerErrorResponse(error, fallbackStatus = 400) {
  ...
  const messages = {
    ...
    PLUGIN_PACKAGE_TRUSTED_HASH_INVALID: "内置插件包的构建验证 SHA-256 无效。",
    PLUGIN_PACKAGE_TRUSTED_HASH_MISMATCH: "内置插件包 SHA-256 与内置目录不一致。",
    PLUGIN_AUTHOR_KEY_ID_INVALID: "作者密钥 ID 无效。",
    PLUGIN_AUTHOR_PUBLIC_KEY_INVALID: "作者公钥不是有效的 Ed25519 JWK。",
    PLUGIN_AUTHOR_PRIVATE_KEY_FORBIDDEN: "禁止上传或保存作者私钥；这里只能使用公钥 JWK。",
    PLUGIN_AUTHOR_STATUS_INVALID: "作者密钥状态无效。",
    PLUGIN_AUTHOR_KEY_NOT_FOUND: "找不到作者公钥。",
    PLUGIN_AUTHOR_KEY_UNTRUSTED: "此作者公钥尚未被信任。",
    PLUGIN_AUTHOR_KEY_REVOKED: "此作者公钥已撤销。",
    PLUGIN_AUTHOR_KEY_SCOPE_DENIED: "此作者密钥未获授权签署该插件 ID。",
    ...
    PLUGIN_SECURITY_OVERRIDE_NOT_REQUIRED: "此项目目前不需要风险例外放行。",
    ...
    PLUGIN_DISTRIBUTION_SCHEMA_UNSUPPORTED: "不支持的分发包 schemaVersion。",
    PLUGIN_DISTRIBUTION_HTTPS_REQUIRED: "外部插件文件／仓库 URL 必须使用 HTTPS。",
    ...
    PLUGIN_DISTRIBUTION_MEDIA_TYPE_UNSUPPORTED: "插件文件媒体类型不受支持。",
    PLUGIN_ARTIFACT_SIZE_MISMATCH: "下载的插件文件大小与签署资料不一致。",
    PLUGIN_ARTIFACT_HASH_MISMATCH: "下载的插件文件 SHA-256 与签署资料不一致。",
    PLUGIN_ARTIFACT_TOO_LARGE: "插件文件超过允许大小。",
    PLUGIN_EXTERNAL_FETCH_FAILED: "无法安全下载外部插件文件。",
    PLUGIN_EXTERNAL_FETCH_HTTP: "外部插件文件下载失败。",
    PLUGIN_QUARANTINE_NOT_FOUND: "找不到隔离区记录。",
  };
...
async function handleV3PackageManagerAuthed(request, env, url, body, session, overrides = {}) {
  ...
  if (quarantineRejectMatch) {
    ...
    try {
      ...
      return jsonResponse({ok:true,entry:safeQuarantineRecord(record),state:await listExternalPluginState(env,overrides),message:"外部插件已标记为已拒绝。"});
    } catch (error) { return packageManagerErrorResponse(error); }
  }
  ...
  if (quarantineDeleteMatch) {
    ...
    try {
      ...
      if (!removed) return jsonResponse({ok:false,code:"PLUGIN_QUARANTINE_NOT_FOUND",message:"找不到隔离区记录。"},404);
  ...
  if (stageMatch) {
    ...
    try {
      ...
      return jsonResponse({ ok:true, transaction:safePackageTransaction(transaction), state:await listPortalPackageState(env, overrides), message:"已建立暂存操作；尚未提交。" });
    } catch (error) { return packageManagerErrorResponse(error); }
  }
  ...
  if (commitMatch) {
    ...
    try {
      ...
      return jsonResponse({ok:true,result:{transaction:safePackageTransaction(result.transaction),package:result.package},state:await listPortalPackageState(env,overrides),message:"安装信息操作已提交；这不代表 JavaScript 运行代码已加载。"});
    } catch (error) { return packageManagerErrorResponse(error); }
  }
  ...
  if (cancelMatch) {
    ...
    try {
      ...
      return jsonResponse({ok:true,state:await listPortalPackageState(env,overrides),message:"暂存操作已取消。"});
    } catch (error) { return packageManagerErrorResponse(error); }
  }
  ...
  if (rollbackMatch) {
    ...
    try {
      ...
      return jsonResponse({ok:true,result:{transaction:safePackageTransaction(result.transaction),package:result.package},state:await listPortalPackageState(env,overrides),message:"安装信息回滚已完成；运行生命周期未自动改变。"});
    } catch (error) { return packageManagerErrorResponse(error); }
  }
