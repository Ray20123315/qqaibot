...
function safeQuarantineRecord(record) {
  ...
  return Object.freeze({
    ...
    author: record.作者 ? Object.freeze({ keyId: String(record.author.keyId || ""), label: String(record.author.label || "") }) : null,
    ...
    security: record.security ? Object.freeze({
      ...
      findings: Object.freeze((record.security.问题数 || []).map(finding => Object.freeze({
...
async function listExternalPluginState(env, overrides = {}) {
  ...
  return Object.freeze({
    ...
    notice: "External/self-made plugin approval verifies 作者 signature and artifact integrity only. Approval does not load JavaScript or grant runtime capabilities."
  });
}
...
function safePackageTransaction(transaction, now = Date.now()) {
  ...
  return Object.freeze({
    ...
    integrity: transaction.integrity ? Object.freeze({
      ...
      范围：String(transaction.integrity.scope || ""),
...
function packageManagerErrorResponse(error, fallbackStatus = 400) {
  ...
  const messages = {
    ...
    PLUGIN_AUTHOR_KEY_已撤销: "此作者 public key 已撤销。",
  ...
  if (pathname === V3_PACKAGE_MANAGER_BASE + "/external/verify") {
    ...
    try {
      ...
      const message = record.state === "blocked"
        ...
        : record.state === "risky"
          ? "簽章與完整性已驗證；偵測到可由安裝者自行承擔的風險，請閱讀 问题数 後再決定是否仍要載入。"
...
function injectV3PackageManagerClient(html) {
  ...
  const section = [
    ...
    '<div class="section-head"><div><h3>受信任插件包</h3><p>这里只管理当前 Worker 已内建/审核候选的 package metadata。Commit 不会动态加载 JavaScript，Plugin Lifecycle 仍独立控制启用状态。</p></div><button id="v3PackageRefresh" class="btn">刷新套件</button></div>',
    ...
    '<div class="section-head"><div><h3>自制／外部插件</h3><p>信任作者 public key，验证 signed distribution 并进入 quarantine。Private key 不上传；Approve 也不会执行 JavaScript。</p></div><button id="v3ExternalRefresh" class="btn">刷新外部插件</button></div>',
