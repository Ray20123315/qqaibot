...
async function authenticatePluginManager(request, env, url, body = {}, overrides = {}) {
  ...
  if (!privileged) {
    return Object.freeze({
      ok: false,
      response: jsonResponse({ ok: false, code: "PLUGIN_MANAGER_DEVELOPER_REQUIRED", message: "只有系统管理员或开发者可以管理 V3 插件。" }, 403)
    });
  }
...
function simplifiedSettingsSchema(schema = {}) {
  const source = schema && typeof schema === "object" && !Array.isArray(schema) ? schema : {};
  return Object.freeze(Object.fromEntries(Object.entries(source).map(([key, descriptor]) => {
    const row = descriptor && typeof descriptor === "object" && !Array.isArray(descriptor) ? { ...descriptor } : descriptor;
    if (!row || typeof row !== "object") return [key, row];
    if (row.label !== undefined) row.label = simplifiedText(row.label);
    if (row.description !== undefined) row.description = simplifiedText(row.description);
    if (Array.isArray(row.options)) row.options = row.options.map(option => {
      if (!option || typeof option !== "object") return option;
      return Object.freeze({ ...option, label: simplifiedText(option.label ?? option.value ?? "") });
    });
    return [key, Object.freeze(row)];
  })));
}

function simplifiedPermissionDisclosures(plugin) {
  return Object.freeze(pluginPermissionDisclosures(plugin).map(row => Object.freeze({
    ...row,
    labelZh: simplifiedText(row?.labelZh || row?.capability || ""),
    descriptionZh: simplifiedText(row?.descriptionZh || "")
  })));
}

function pluginSummary(plugin, surface = null) {
  const schema = simplifiedSettingsSchema(plugin?.settings && typeof plugin.settings === "object" ? plugin.settings : {});
  ...
  return Object.freeze({
    id: String(plugin?.id || ""),
    name: simplifiedText(plugin?.name || ""),
    version: String(plugin?.version || ""),
    apiVersion: String(plugin?.apiVersion || ""),
    description: simplifiedText(plugin?.description || ""),
    author: String(plugin?.author || ""),
    official: plugin?.official === true,
    trustStatus,
    trustLabelZh: simplifiedText(pluginTrustLabelZh(trustStatus)),
    releaseChannel,
    releaseChannelLabelZh: simplifiedText(releaseChannelLabelZh(releaseChannel)),
    ...
    permissionDisclosures: simplifiedPermissionDisclosures(plugin),
...
async function handleV3PluginManagerAuthed(request, env, url, body, session, overrides = {}) {
  ...
  if (stateMatch) {
    ...
    if (!runtimeState.runtimeEnabled) {
      return jsonResponse({ ok: false, code: "V3_RUNTIME_DISABLED", message: "V3 运行环境目前关闭，不能修改插件生命周期。" }, 409);
    }
  ...
  if (permissionsMatch) {
    ...
    if (!runtimeState.runtimeEnabled) {
      return jsonResponse({ ok: false, code: "V3_RUNTIME_DISABLED", message: "V3 运行环境目前关闭，不能修改插件权限。" }, 409);
    }
    ...
    try {
      ...
      return jsonResponse({
        ...
        message: lifecycle?.state === "blocked"
          ? "权限已更新；插件目前缺少必要权限，因此仍被阻挡。"
          : (lifecycle?.degraded ? "权限已更新；插件仍可启用，但部分未授权功能会停用。" : "插件权限已更新。")
      });
    } catch (error) {
      return pluginManagerErrorResponse(error);
    }
  ...
  if (channelMatch) {
    ...
    try {
      ...
      let surface = null;
      try { surface = await runtime.getPluginSurface(pluginId, { userId: session.qq }); } catch {}
      return jsonResponse({ ok: true, plugin: pluginSummary(plugin, surface), lifecycle, message: channel === "preview" ? "已切换为抢先体验版优先。" : "已切换为稳定版优先。" });
  ...
  if (settingsMatch) {
    ...
    if (!runtimeState.runtimeEnabled) {
      return jsonResponse({ ok: false, code: "V3_RUNTIME_DISABLED", message: "V3 运行环境目前关闭，不能修改插件设置。" }, 409);
    }
  ...
  if (detailMatch) {
    ...
    if (!runtimeState.runtimeEnabled) {
      return jsonResponse({ ok: false, code: "V3_RUNTIME_DISABLED", message: "V3 运行环境目前关闭。" }, 409);
    }
...
function injectV3PluginManagerClient(html) {
  ...
  const section = [
    ...
    '<div class="section-head"><div><h2>V3 插件管理</h2><p>列表仅显示插件摘要；权限、版本、设置与运行状态请在“查看详情”中管理。</p></div><button id="v3PluginRefresh" class="btn">重新加载</button></div>',
    ...
    '</section>',
    '<div id="v3PluginModal" class="v3-plugin-modal hidden" aria-hidden="true" role="dialog" aria-modal="true" aria-labelledby="v3PluginModalTitle">',
    '<div class="v3-plugin-modal-card">',
    '<div class="v3-plugin-modal-head"><div class="v3-plugin-modal-heading"><h3 id="v3PluginModalTitle">插件详情</h3><div id="v3PluginModalMeta" class="item-meta"></div></div><button id="v3PluginModalClose" class="btn" type="button" aria-label="关闭插件详情">关闭</button></div>',
    '<div id="v3PluginModalBody" class="v3-plugin-modal-body"><div class="empty">正在读取插件详情。</div></div>',
    '</div>',
    '</div>',
    '</div>'
  ].join("");
  ...
  const style = '<style id="qqai-v3-plugin-manager-style">'
    + '.v3-plugin-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr));gap:14px;margin-top:16px}'
    + '.v3-plugin-card{background:var(--panel);border:1px solid var(--line);border-radius:17px;padding:18px;box-shadow:0 8px 28px rgba(38,49,76,.045);min-width:0;display:flex;flex-direction:column;gap:0}'
    + '.v3-plugin-title{display:flex;gap:10px;align-items:flex-start;justify-content:space-between}.v3-plugin-title h3{margin:0;font-size:17px}'
    + '.v3-plugin-badges{display:flex;gap:6px;flex-wrap:wrap;margin-top:8px}.v3-plugin-badge{display:inline-flex;padding:3px 7px;border-radius:999px;background:var(--panel2);font-size:11px;font-weight:800}'
    + '.v3-plugin-card .item-body{margin-top:12px;line-height:1.6}.v3-plugin-card-actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:16px}'
    + '.v3-plugin-setting{margin-top:12px}.v3-plugin-setting textarea{min-height:130px;font-family:ui-monospace,SFMono-Regular,Consolas,monospace}'
    + '.v3-plugin-actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:14px}.v3-plugin-status{margin-top:14px}.v3-plugin-status pre{margin:8px 0 0;padding:10px;border-radius:10px;background:var(--panel2);max-height:260px;overflow:auto;font-size:11px}'
    + '.v3-plugin-runtime-off{border-color:#d97706}.v3-plugin-runtime-on{border-color:#16a34a}'
    + '.v3-plugin-modal{position:fixed;inset:0;z-index:1200;background:rgba(8,12,20,.66);backdrop-filter:blur(8px);display:flex;align-items:center;justify-content:center;padding:24px}'
    + '.v3-plugin-modal.hidden{display:none!important}.v3-plugin-modal-card{width:min(920px,100%);max-height:min(90vh,900px);background:var(--panel);border:1px solid var(--line);border-radius:20px;box-shadow:0 28px 80px rgba(0,0,0,.32);display:flex;flex-direction:column;overflow:hidden}'
    + '.v3-plugin-modal-head{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;padding:18px 20px;border-bottom:1px solid var(--line);background:var(--panel)}'
    + '.v3-plugin-modal-heading{min-width:0}.v3-plugin-modal-heading h3{margin:0;font-size:20px}.v3-plugin-modal-body{padding:20px;overflow:auto;overscroll-behavior:contain}'
    + '.v3-plugin-detail-summary{display:flex;gap:10px;align-items:flex-start;justify-content:space-between;flex-wrap:wrap}.v3-plugin-detail-section{margin-top:18px}.v3-plugin-detail-section>h4{margin:0 0 10px;font-size:15px}'
    + '@media(max-width:700px){.v3-plugin-grid{grid-template-columns:1fr}.v3-plugin-card{padding:15px}.v3-plugin-modal{padding:0;align-items:flex-end}.v3-plugin-modal-card{width:100%;max-height:94dvh;border-radius:20px 20px 0 0}.v3-plugin-modal-head{padding:15px 16px}.v3-plugin-modal-body{padding:16px}}'
    + '</style>';
  const lifecycleStyle = '<style id="qqai-v3-plugin-lifecycle-style">'
    + '.v3-lifecycle-box{margin-top:14px;padding:12px;border:1px solid var(--line);border-radius:13px;background:var(--panel2)}'
    + '.v3-lifecycle-head{display:flex;gap:8px;justify-content:space-between;align-items:center;flex-wrap:wrap}'
    + '.v3-lifecycle-perms{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:7px;margin-top:10px}'
    + '.v3-lifecycle-perm{display:flex;gap:7px;align-items:flex-start;font-size:12px}'
    + '.v3-lifecycle-reason{margin-top:8px;font-size:12px;color:var(--warn)}'
    + '.v3-lifecycle-actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:10px}'
    + '</style>';
  source = source.includes("</head>") ? source.replace("</head>", style + lifecycleStyle + "</head>") : style + lifecycleStyle + source;
  ...
  const script = "<script id=\"qqai-v3-plugin-manager-client\">\n(function(){\n'use strict';\n"
    ...
    + "var modal=document.getElementById('v3PluginModal');\n"
    + "var modalTitle=document.getElementById('v3PluginModalTitle');\n"
    + "var modalMeta=document.getElementById('v3PluginModalMeta');\n"
    + "var modalBody=document.getElementById('v3PluginModalBody');\n"
    ...
    + "  return '<div class=\\\"v3-lifecycle-box\\\"><div class=\\\"v3-lifecycle-head\\\"><b>生命周期与权限</b><span class=\\\"status '+statusClass(stateName)+'\\\">'+esc(statusLabel(stateName))+'</span></div><div class=\\\"item-meta\\\">'+meta+'</div>'+reason+(requested.length?'<div class=\\\"v3-lifecycle-perms\\\">'+perms+'</div><div class=\\\"v3-lifecycle-actions\\\">'+toggle+'<button class=\\\"btn\\\" data-v3-save-permissions=\\\"'+esc(p.id)+'\\\">保存权限</button>'+channel+'</div>':'<div class=\\\"v3-lifecycle-actions\\\">'+toggle+channel+'</div>')+'</div>';\n"
    + "}\n"
    + "function pluginBadges(p){var caps=(p.capabilities||[]).map(function(x){return'<span class=\\\"v3-plugin-badge\\\">'+esc(x)+'</span>'}).join('');return'<span class=\\\"v3-plugin-badge\\\">'+esc(p.trustLabelZh||'尚未取得认证')+'</span><span class=\\\"v3-plugin-badge\\\">'+esc(p.releaseChannelLabelZh||'稳定版')+'</span>'+(p.publicStatus?'<span class=\\\"v3-plugin-badge\\\">公开状态</span>':'')+caps}\n"
    + "function renderPlugin(p){\n"
    + "  var status=p.status||{},badge='<span class=\\\"status '+statusClass(status.state)+'\\\">'+esc(statusLabel(status.state||'UNKNOWN'))+'</span>';\n"
    + "  return '<article class=\\\"v3-plugin-card\\\" data-v3-plugin=\\\"'+esc(p.id)+'\\\"><div class=\\\"v3-plugin-title\\\"><div><h3>'+esc(p.name||p.id)+'</h3><div class=\\\"item-meta\\\">'+esc(p.id)+'｜v'+esc(p.version)+'｜插件 API '+esc(p.apiVersion)+'</div></div>'+badge+'</div><div class=\\\"v3-plugin-badges\\\">'+pluginBadges(p)+'</div>'+(p.description?'<div class=\\\"item-body\\\">'+esc(p.description)+'</div>':'')+'<div class=\\\"v3-plugin-card-actions\\\"><button class=\\\"btn primary\\\" data-v3-open-detail=\\\"'+esc(p.id)+'\\\">查看详情</button></div></article>';\n"
    + "}\n"
    + "function renderPluginDetail(p){\n"
    + "  var schema=p.settingsSchema||{},settings=p.settings||{},status=p.status||{},fields=Object.keys(schema).map(function(k){return settingControl(p,k,schema[k],settings[k])}).join('');\n"
    ...
    + "  return '<div class=\\\"v3-plugin-detail-summary\\\"><div class=\\\"v3-plugin-badges\\\">'+pluginBadges(p)+'</div><span class=\\\"status '+statusClass(status.state)+'\\\">'+esc(statusLabel(status.state||'UNKNOWN'))+'</span></div>'+(p.description?'<div class=\\\"item-body\\\">'+esc(p.description)+'</div>':'')+lifecycleControl(p)+'<div class=\\\"v3-plugin-detail-section\\\"><h4>插件设置</h4>'+(fields||'<div class=\\\"notice\\\">此插件没有可配置设置。</div>')+'<div class=\\\"v3-plugin-actions\\\">'+save+'<button class=\\\"btn\\\" data-v3-detail-refresh=\\\"'+esc(p.id)+'\\\">重新读取状态</button></div></div><details class=\\\"v3-plugin-status\\\"><summary>查看运行状态</summary><pre>'+esc(JSON.stringify(status,null,2))+'</pre></details>';\n"
    + "}\n"
    + "function closeDetail(){activeDetailId='';modal.classList.add('hidden');modal.setAttribute('aria-hidden','true');document.body.style.overflow=''}\n"
    + "function showDetail(p){if(!p)return;activeDetailId=String(p.id||'');modalTitle.textContent=p.name||p.id||'插件详情';modalMeta.textContent=String(p.id||'')+'｜v'+String(p.version||'')+'｜插件 API '+String(p.apiVersion||'');modalBody.innerHTML=renderPluginDetail(p);modal.classList.remove('hidden');modal.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';bindDetailControls();if(modalClose)modalClose.focus()}\n"
    + "function bindCards(){list.querySelectorAll('[data-v3-open-detail]').forEach(function(btn){btn.onclick=function(){openDetail(this.dataset.v3OpenDetail)}})}\n"
    + "function bindDetailControls(){\n"
    + "  modalBody.querySelectorAll('[data-v3-save]').forEach(function(btn){btn.onclick=function(){savePlugin(this.dataset.v3Save)}});\n"
    + "  modalBody.querySelectorAll('[data-v3-detail-refresh]').forEach(function(btn){btn.onclick=function(){loadDetail(this.dataset.v3DetailRefresh,true)}});\n"
    + "  modalBody.querySelectorAll('[data-v3-toggle]').forEach(function(btn){btn.onclick=function(){togglePlugin(this.dataset.v3Toggle,this.dataset.enabled==='true')}});\n"
    + "  modalBody.querySelectorAll('[data-v3-save-permissions]').forEach(function(btn){btn.onclick=function(){savePermissions(this.dataset.v3SavePermissions)}});\n"
    + "  modalBody.querySelectorAll('[data-v3-save-channel]').forEach(function(btn){btn.onclick=function(){saveChannel(this.dataset.v3SaveChannel)}});\n"
    + "}\n"
    ...
    + "}\n"
    + "async function openDetail(id){var cached=pluginCache[id];if(cached)showDetail(cached);else{activeDetailId=id;modalTitle.textContent='插件详情';modalMeta.textContent='';modalBody.innerHTML='<div class=\\\"empty\\\">正在读取插件详情。</div>';modal.classList.remove('hidden');modal.setAttribute('aria-hidden','false');document.body.style.overflow='hidden'}await loadDetail(id,true)}\n"
    + "async function loadDetail(id,keepOpen){var r=await req('/'+encodeURIComponent(id));if(!r.data.ok){if(keepOpen){modalBody.innerHTML='<div class=\\\"empty\\\">'+esc(r.data.message||'状态读取失败')+'</div>'}else alert(r.data.message||'状态读取失败');return}pluginCache[id]=r.data.plugin;var card=list.querySelector('[data-v3-plugin=\\\"'+CSS.escape(id)+'\\\"]');if(card){var fresh=document.createElement('div');fresh.innerHTML=renderPlugin(r.data.plugin);card.replaceWith(fresh.firstElementChild);bindCards()}if(keepOpen||activeDetailId===id)showDetail(r.data.plugin)}\n"
    + "function detailRoot(id){return activeDetailId===id?modalBody:null}\n"
    + "function collectSettings(root){var out={};root.querySelectorAll('[data-v3-setting]').forEach(function(el){var key=el.dataset.v3Setting,type=el.dataset.v3Type;if(el.disabled)return;if(type==='boolean')out[key]=!!el.checked;else if(type==='number'){if(el.value!=='')out[key]=Number(el.value)}else if(type==='json')out[key]=JSON.parse(el.value||'null');else if(type==='string'&&el.type==='password'&&!el.value){}else out[key]=el.value});return out}\n"
    + "async function savePlugin(id){var root=detailRoot(id);if(!root)return;var settings;try{settings=collectSettings(root)}catch(e){alert('JSON 设置格式错误：'+e.message);return}var r=await req('/'+encodeURIComponent(id)+'/settings','POST',{settings:settings});alert(r.data.message||'保存完成');if(r.data.ok)await loadDetail(id,true)}\n"
    + "async function togglePlugin(id,enabled){if(!confirm(enabled?'确定启用此插件？':'确定停用此插件？'))return;var r=await req('/'+encodeURIComponent(id)+'/state','POST',{enabled:enabled});alert(r.data.message||'操作完成');if(r.data.ok)await loadDetail(id,true)}\n"
    + "async function savePermissions(id){var root=detailRoot(id);if(!root)return;var permissions=Array.from(root.querySelectorAll('[data-v3-permission]:checked')).map(function(el){return el.dataset.v3Permission});var r=await req('/'+encodeURIComponent(id)+'/permissions','PUT',{permissions:permissions});alert(r.data.message||'权限已更新');if(r.data.ok)await loadDetail(id,true)}\n"
    + "async function saveChannel(id){var root=detailRoot(id);if(!root)return;var el=root.querySelector('[data-v3-channel]');if(!el)return;var r=await req('/'+encodeURIComponent(id)+'/release-channel','PUT',{channel:el.value});alert(r.data.message||'版本偏好已更新');if(r.data.ok)await loadDetail(id,true)}\n"
    + "if(modalClose)modalClose.onclick=closeDetail;\n"
    + "modal.addEventListener('click',function(e){if(e.target===modal)closeDetail()});\n"
    + "document.addEventListener('keydown',function(e){if(e.key==='Escape'&&!modal.classList.contains('hidden'))closeDetail()});\n"
