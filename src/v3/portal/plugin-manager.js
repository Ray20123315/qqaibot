...
function injectV3PluginManagerClient(html) {
  ...
  const script = '<script id="qqai-v3-plugin-manager-client">(function(){\\'use strict\\';var nav=document.getElementById(\\'v3PluginManagerNav\\'),app=document.getElementById(\\'app\\'),list=document.getElementById(\\'v3PluginList\\'),state=document.getElementById(\\'v3PluginRuntimeState\\'),refresh=document.getElementById(\\'v3PluginRefresh\\');if(!nav||!list||!state)return;function esc(v){return String(v==null?\\'\\':v).replace(/[&<>"\\']/g,function(c){return{\\'&\\':\\'&amp;\\',\\'<\\':\\'&lt;\\',\\'>\\':\\'&gt;\\',\\'"\\':\\'&quot;\\',"\\'":\\'&#39;\\'}[c]})}function statusClass(v){v=String(v||\\'\\').toUpperCase();return v===\\'OK\\'?\\'ok\\':v===\\'DEGRADED\\'?\\'warning\\':v===\\'DISABLED\\'?\\'warning\\':\\'error\\'}async function req(path,method,body){var init={method:method||\\'GET\\',headers:{Accept:\\'application/json\\'}};if(body!==undefined){init.headers[\\'Content-Type\\']=\\'application/json\\';init.body=JSON.stringify(body)}var res=await fetch(\\'/api/portal/v3/plugins\\'+(path||\\'\\'),init),data={};try{data=await res.json()}catch(e){}return{status:res.status,data:data}}function settingControl(plugin,key,d,value){var id=\\'v3ps-\\'+plugin.id.replace(/[^a-z0-9_-]/gi,\\'-\\')+\\'-\\'+key,label=esc(d.label||key),desc=d.description?\\'<div class="item-meta">\\'+esc(d.description)+\\'</div>\\':\\'\\',control=\\'\\';if(d.type===\\'boolean\\'){control=\\'<label class="switch"><input id="\\'+id+\\'" data-v3-setting="\\'+esc(key)+\\'" data-v3-type="boolean" type="checkbox" \\'+(value===true?\\'checked\\':\\'\\')+\\'> \\'+label+\\'</label>\\'+desc}else if(d.type===\\'select\\'){control=\\'<div class="field v3-plugin-setting"><label>\\'+label+\\'</label><select id="\\'+id+\\'" data-v3-setting="\\'+esc(key)+\\'" data-v3-type="select">\\'+(d.options||[]).map(function(o){return\\'<option value="\\'+esc(o.value)+\\'" \\'+(String(value)==String(o.value)?\\'selected\\':\\'\\')+\\'>\\'+esc(o.label||o.value)+\\'</option>\\'}).join(\\'\\')+\\'</select>\\'+desc+\\'</div>\\'}else if(d.type===\\'json\\'){control=\\'<div class="field v3-plugin-setting"><label>\\'+label+\\'</label><textarea id="\\'+id+\\'" data-v3-setting="\\'+esc(key)+\\'" data-v3-type="json" \\'+(d.readOnly?\\'disabled\\':\\'\\')+\\'>\\'+esc(JSON.stringify(value==null?null:value,null,2))+\\'</textarea>\\'+desc+\\'</div>\\'}else{var type=d.secret?\\'password\\':(d.type===\\'number\\'?\\'number\\':\\'text\\'),val=d.secret&&value===\\'[redacted]\\'?\\'\\':(value==null?\\'\\':String(value)),attrs=d.type===\\'number\\'?\\' min="\\'+esc(d.min==null?\\'\\':d.min)+\\'" max="\\'+esc(d.max==null?\\'\\':d.max)+\\'" step="\\'+esc(d.step==null?\\'any\\':d.step)+\\'"\\':\\'\\';control=\\'<div class="field v3-plugin-setting"><label>\\'+label+(d.secret?\\'（secret）\\':\\'\\')+\\'</label><input id="\\'+id+\\'" data-v3-setting="\\'+esc(key)+\\'" data-v3-type="\\'+esc(d.type)+\\'" type="\\'+type+\\'" value="\\'+esc(val)+\\'" \\'+attrs+\\' \\'+(d.readOnly?\\'disabled\\':\\'\\')+\\' placeholder="\\'+(d.secret&&value===\\'[redacted]\\'?\\'已設定；留空代表不覆蓋\\':\\'\\')+\\'">\\'+desc+\\'</div>\\'}return control}function renderPlugin(p){var schema=p.settingsSchema||{},settings=p.settings||{},status=p.status||{},badge=\\'<span class="status \\'+statusClass(status.state)+\\'">\\'+esc(status.state||\\'UNKNOWN\\')+\\'</span>\\',caps=(p.capabilities||[]).map(function(x){return\\'<span class="v3-plugin-badge">\\'+esc(x)+\\'</span>\\'}).join(\\'\\'),fields=Object.keys(schema).map(function(k){return settingControl(p,k,schema[k],settings[k])}).join(\\'\\'),save=p.surface&&p.surface.writableSettings?\\'<button class="btn primary" data-v3-save="\\'+esc(p.id)+\\'">保存设置</button>\\':\\'<button class="btn" disabled>设置只读</button>\\';return\\'<article class="v3-plugin-card" data-v3-plugin="\\'+esc(p.id)+\\'"><div class="v3-plugin-title"><div><h3>\\'+esc(p.name||p.id)+\\'</h3><div class="item-meta">\\'+esc(p.id)+\\'｜v\\'+esc(p.version)+\\'｜Plugin API \\'+esc(p.apiVersion)+\\'</div></div>\\'+badge+\\'</div><div class="v3-plugin-badges">\\'+(p.official?\\'<span class="v3-plugin-badge">官方</span>\\':\\'<span class="v3-plugin-badge">第三方</span>\\')+(p.publicStatus?\\'<span class="v3-plugin-badge">Public Status</span>\\':\\'\\')+caps+\\'</div>\\'+(p.description?\\'<div class="item-body">\\'+esc(p.description)+\\'</div>\\':\\'\\')+(fields||\\'<div class="notice">此插件没有可配置设置。</div>\\')+\\'<div class="v3-plugin-actions">\\'+save+\\'<button class="btn" data-v3-detail="\\'+esc(p.id)+\\'">重新读取状态</button></div><details class="v3-plugin-status"><summary>查看运行状态</summary><pre>\\'+esc(JSON.stringify(status,null,2))+\\'</pre></details></article>\\'}function bindCards(){list.querySelectorAll(\\'[data-v3-save]\\').forEach(function(btn){btn.onclick=function(){savePlugin(this.dataset.v3Save)}});list.querySelectorAll(\\'[data-v3-detail]\\').forEach(function(btn){btn.onclick=function(){loadDetail(this.dataset.v3Detail)}})}async function loadPlugins(){state.textContent=\\'正在读取 V3 Runtime 与插件状态…\\';var r=await req(\\'\\');if(r.status===401||r.status===403){nav.hidden=true;return}nav.hidden=false;if(!r.data.ok){state.textContent=r.data.message||\\'插件状态读取失败。\\';list.innerHTML=\\'<div class="empty">\\'+esc(r.data.message||\\'加载失败\\')+\\'</div>\\';return}var enabled=!!r.data.runtimeEnabled;state.className=\\'notice \\'+(enabled?\\'v3-plugin-runtime-on\\':\\'v3-plugin-runtime-off\\');state.textContent=enabled?\\'V3 Runtime 已启用｜已载入 \\'+Number(r.data.pluginCount||0)+\\' 个插件\\'+(r.data.bilibiliEnabled?\\'｜Bilibili 官方插件已启用\\':\\'\\'):\\'V3 Runtime 目前关闭（V3_RUNTIME_ENABLED=false）。此页面只显示安全状态，不会因为打开 Portal 而启动 V3 或扫描 D1。\\';list.innerHTML=(r.data.plugins||[]).map(renderPlugin).join(\\'\\')||(enabled?\\'<div class="empty">Runtime 已启用，但目前没有载入插件。</div>\\':\\'<div class="empty">V3 尚未启用，因此没有运行中的插件。</div>\\');bindCards()}async function loadDetail(id){var r=await req(\\'/\\'+encodeURIComponent(id));if(!r.data.ok){alert(r.data.message||\\'状态读取失败\\');return}var card=list.querySelector(\\'[data-v3-plugin="\\'+CSS.escape(id)+\\'"]\\');if(card){var fresh=document.createElement(\\'div\\');fresh.innerHTML=renderPlugin(r.data.plugin);card.replaceWith(fresh.firstElementChild);bindCards()}}function collectSettings(card){var out={};card.querySelectorAll(\\'[data-v3-setting]\\').forEach(function(el){var key=el.dataset.v3Setting,type=el.dataset.v3Type;if(el.disabled)return;if(type===\\'boolean\\')out[key]=!!el.checked;else if(type===\\'number\\'){if(el.value!==\\'\\')out[key]=Number(el.value)}else if(type===\\'json\\'){out[key]=JSON.parse(el.value||\\'null\\')}else if(type===\\'string\\'&&el.type===\\'password\\'&&!el.value){}else out[key]=el.value});return out}async function savePlugin(id){var card=list.querySelector(\\'[data-v3-plugin="\\'+CSS.escape(id)+\\'"]\\');if(!card)return;var settings;try{settings=collectSettings(card)}catch(e){alert(\\'JSON 设置格式错误：\\'+e.message);return}var r=await req(\\'/\\'+encodeURIComponent(id)+\\'/settings\\',\\'POST\\',{settings:settings});alert(r.data.message||\\'保存完成\\');if(r.data.ok)await loadDetail(id)}nav.addEventListener(\\'click\\',function(){setTimeout(function(){var title=document.getElementById(\\'pageTitle\\');if(title)title.textContent=\\'插件管理\\';loadPlugins()},0)});if(refresh)refresh.onclick=loadPlugins;function probe(){if(!app||app.classList.contains(\\'hidden\\'))return;loadPlugins()}if(app){new MutationObserver(function(){probe()}).observe(app,{attributes:true,attributeFilter:[\\'class\\']});if(!app.classList.contains(\\'hidden\\'))probe()}})();</script>';
