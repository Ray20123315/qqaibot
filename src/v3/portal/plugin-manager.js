  ...
  if (permissionsMatch) {
    ...
    try {
      ...
      return jsonResponse({
        ...
        plugin: pluginSummary(plugin, surface),
        lifecycle,
        message: lifecycle?.state === "blocked" ? "权限已更新；插件目前因缺少必要权限而处于 blocked。" : "插件权限已更新。"
      });
    } catch (error) {
      return pluginManagerErrorResponse(error);
    }
