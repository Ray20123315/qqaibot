...
assert.notEqual(db.get("conversation:123:c_orphan"), null, "lean cleanup does not scan legacy conversation payloads");
assert.notEqual(db.get("ai_decision_log:ai_keep"), null);
assert.notEqual(db.get("ai_decision_log:ai_orphan"), null, "lean cleanup does not scan disabled AI decision logs");
assert.notEqual(db.get("platform:trace:tr_keep"), null);
assert.notEqual(db.get("platform:trace:tr_orphan"), null, "lean cleanup does not scan platform trace payloads");
assert.notEqual(db.get("platform:job:job_live"), null);
assert.notEqual(db.get("platform:job:job_orphan"), null, "lean cleanup does not scan disabled platform jobs");
assert.notEqual(db.get("platform:job:job_unindexed_live"), null);
const cleanupSql = db.preparedSql.join("\n");
assert.doesNotMatch(cleanupSql, /conversation:index:|ai_decision_log:index|platform:trace:index|platform:job:index/, "hourly cleanup must avoid legacy orphan-index joins");
