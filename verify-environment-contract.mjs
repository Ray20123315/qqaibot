...
assert.match(registerBlock, /createPortalAdminAccountBinding\(env, \{ qq \}\)/);
assert.match(registerBlock, /developer_admin_password_setup/);
assert.match(registerBlock, /verifyPortalVerificationCode/);
assert.doesNotMatch(registerBlock, /setupKey|developerPortalBootstrap|PORTAL_AUTH_SECRET|ONEBOT_ACCESS_TOKEN/);
assert.doesNotMatch(worker, /developerPortalBootstrapSecrets|developerPortalBootstrapPolicy|DEVELOPER_BOOTSTRAP_SECRET/);
assert.match(registerBlock, /let activationStage = "identity_verification"/);
assert.match(registerBlock, /ACCOUNT_ACTIVATED_LOGIN_REQUIRED/);
assert.match(registerBlock, /classifyPortalAuthFailure/);
assert.match(registerBlock, /failureId/);
