...
const base = '<!doctype html><html><head></head><body><nav><button data-view="overview">首页</button><div id="pluginCompatNav" class="plugin-compat-nav" hidden></div></nav><main><section id="v-logs"></section></main></body></html>';
...
for (const glyph of ['友','禁','关','资','清']) assert(html.includes('qqai-nav-glyph') && html.includes('>'+glyph+'<'), 'Missing plugin bridge glyph '+glyph);
assert(/id="pluginCompatNav"[\s\S]*id="memberConsoleNav"/.test(html), 'Member navigation must stay inside pluginCompatNav');
...
assert(pkg.version === '2.7.12' && pkg.scripts.check.includes('verify-member-console-split.mjs'), 'v2.7.3 permanent check must be registered');
