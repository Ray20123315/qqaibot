...
assert(result.body.includes('class="cyber-skyline"'), 'GET /: cyber skyline layer missing');
assert(result.body.includes('class="preview-search"'), 'GET /: dashboard search preview missing');
assert(result.body.includes('class="preview-stat-grid"'), 'GET /: dashboard status density missing');
assert(result.body.includes('class="preview-resource-rail"'), 'GET /: resource rail missing');
assert(result.body.includes('class="roles-layout"'), 'GET /: dense roles layout missing');
assert(result.body.includes('KV / R2 目前為未來 BYOR 預留'), 'GET /: future resource disclaimer missing');
