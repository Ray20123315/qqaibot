...
const injectedClient = injected.match(/<script id="qqai-v3-plugin-manager-client">([\\s\\S]*?)<\\/script>/);\nassert.ok(injectedClient, "injected Plugin Manager client script must exist");\nassert.doesNotThrow(() => new Function(injectedClient[1]), "injected Plugin Manager browser client must compile");\nassert.match(injected, /qqai-v3-plugin-lifecycle-style/);
assert.match(injected, /id="v3PluginModal"/);
assert.match(injected, /class="v3-plugin-modal hidden"/);
assert.match(injected, /data-v3-open-detail/);
assert.match(injected, /查看详情/);
assert.match(injected, /function renderPluginDetail/);
assert.match(injected, /function closeDetail/);
assert.match(injected, /e\.key===\'Escape\'/);
const compactRender = injected.match(/function renderPlugin\(p\)\{([\s\S]*?)\n\}\nfunction renderPluginDetail/);
assert.ok(compactRender, "compact plugin-card renderer must be present before detail renderer");
assert.match(compactRender[1], /data-v3-open-detail/);
assert.doesNotMatch(compactRender[1], /lifecycleControl\(p\)|settingControl\(|data-v3-save=|data-v3-toggle|data-v3-permission|v3-plugin-status/);
