...
async function handlePortalApi(request, env, url) {
  ...
  if (request.method === "GET" && path === "/settings") {
    return jsonResponse({
      ...
      quota: await dbGet(env, `quota:${groupId}:${authed.qq}`) || "无限"
    });
  }
  ...
  if (request.method === "GET" && path === "/root/members") {
    ...
    for (const member of members) {
      const qq = String(member.qq);
      enriched.push({
        ...member,
        granted_admin: await dbGet(env, `admin_auth:${qq}`) === "true",
        memory_banned: await dbGet(env, `memory_banned:${qq}`) === "true",
        quota: await dbGet(env, `quota:${groupId}:${qq}`) || "无限"
      });
    }
  ...
  if (request.method === "POST" && path === "/root/member") {
    ...
    if (body.quota !== undefined) await dbPut(env, `quota:${groupId}:${target}`, String(body.quota || "无限"));
...
<head>
  ...
  <style>
    *{box-sizing:border-box}html,body{margin:0;min-height:100%;font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:#03050a;color:#edf6ff}
    ...
    label{display:block;font-size:13px;color:#9fb1c7;margin:14px 0 6px}input,select{width:100%;height:40px;border:1px solid rgba(255,255,255,.18);border-radius:6px;background:#09111f;color:#eef6ff;padding:0 10px}input[type=checkbox]{width:18px;height:18px;padding:0;accent-color:#57d7ff;flex:0 0 auto}
    ...
    .dashboard{display:none;position:relative;z-index:1;padding:22px 32px 34px;min-height:100vh}.dashTop{display:flex;justify-content:space-between;gap:14px;align-items:end;margin-bottom:14px}.dashTop h1{margin:0;font-size:30px}.role{color:#8be9ff}.tabs{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px}.tab{background:#132033;color:#d9ecff}.view{display:none}.view.active{display:block}.grid{display:grid;grid-template-columns:repeat(12,1fr);gap:12px}.mini{grid-column:span 3;border:1px solid rgba(255,255,255,.12);border-radius:8px;padding:14px;background:rgba(8,16,29,.88);min-width:0}.wide{grid-column:span 6}.full{grid-column:1/-1}.muted{color:#9fb1c7;font-size:14px;line-height:1.6}.list{display:grid;gap:8px;margin-top:10px;max-height:320px;overflow:auto}.item{display:flex;gap:8px;align-items:center;justify-content:space-between;border:1px solid rgba(255,255,255,.1);background:#07101b;border-radius:6px;padding:8px;min-height:38px}.item span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.audit{display:block}.audit small{display:block;color:#9fb1c7;margin-top:4px;white-space:normal}.danger{background:#ff6b6b;color:#170707}.ok{background:#73e6a2;color:#03120a}textarea{width:100%;min-height:84px;border:1px solid rgba(255,255,255,.18);border-radius:6px;background:#09111f;color:#eef6ff;padding:10px}input[type=range]{width:100%}.rangeLine{display:grid;grid-template-columns:1fr 58px;gap:10px;align-items:center}.percent{font-variant-numeric:tabular-nums;color:#8be9ff;font-weight:800}.switch{display:flex;gap:8px;align-items:center;margin:8px 0}.matrixCanvas{height:330px;border:1px solid rgba(255,255,255,.12);border-radius:8px;background:#030914;position:relative;overflow:hidden}.toolbar{display:flex;gap:8px;flex-wrap:wrap;align-items:center}.toolbar input{max-width:280px}.stat{font-size:24px;font-weight:800;color:#8be9ff}.smallBtn{height:34px;padding:0 10px}.tip{display:none;position:fixed;z-index:9;max-width:min(420px,90vw);background:rgba(3,8,15,.95);border:1px solid rgba(139,233,255,.45);border-radius:8px;padding:10px;color:#edf6ff;box-shadow:0 8px 30px rgba(0,0,0,.35);pointer-events:none}
...
<body>
  ...
  <section class="dashboard" id="dashboard">
    ...
    <div class="view active" id="view-self">
      <div class="grid">
        ...
        <div class="mini"><strong>個人設定</strong><label class="switch"><input id="dnd" type="checkbox"> 免打擾</label><textarea id="style" placeholder="私人專屬 AI 態度"></textarea><div class="row"><button id="saveSettings">保存</button></div><p class="muted">配額：<span class="stat" id="quotaText">無限</span></p></div>
  ...
  <script>
    ...
    async function loadSelf(){const mem=await getApi('/api/portal/memories');banHint.textContent=mem.memory_banned?'【操作失敗：您的記憶編輯權限已被管理員凍結】':'';renderList(privateMems,mem.private,'private');renderList(publicMems,mem.public,'public');const st=await getApi('/api/portal/settings');dnd.checked=!!st.dnd;style.value=st.style||'';quotaText.textContent=st.quota||'無限'}
    function auditHtml(l){return '<div class="item audit"><span>'+escapeHtml(l.at+' / QQ:'+l.userId+' / '+l.action)+'</span><small>變更前：'+escapeHtml(formatAudit(l.before))+'</small><small>變更後：'+escapeHtml(formatAudit(l.after))+'</small></div>'}
    function formatAudit(v){if(v===null||v===undefined||v==='')return '無';try{const parsed=JSON.parse(v);return typeof parsed==='string'?parsed:JSON.stringify(parsed)}catch(e){return String(v)}}
    function syncInterjectLabel(){interjectValue.textContent=interjectRate.value+'%'}
    async function loadAdmin(){if(!canAdmin()){alert('Error 403：權限不足');return}const s=await getApi('/api/portal/admin/state');aiOn.checked=s.ai_on;memoryOn.checked=s.memory_on;groupPersona.value=s.persona||'';interjectRate.value=s.interject_rate||25;syncInterjectLabel();keywords.value=(s.keywords||[]).join('\\n');blacklist.innerHTML=(s.blacklist||[]).map(q=>'<div class="item"><span>QQ:'+q+'</span></div>').join('');auditLogs.innerHTML=(s.audit_logs||[]).slice(-30).reverse().map(auditHtml).join('')||'<div class="muted">目前沒有審計紀錄</div>'}
    async function loadRoot(){if(!canRoot()){alert('Error 403：權限不足');return}const res=await getApi('/api/portal/root/members');memberList.innerHTML=(res.members||[]).map(m=>'<div class="item"><span>'+escapeHtml((m.name||m.qq)+' QQ:'+m.qq+' / '+m.role+' / Admin授權:'+(m.granted_admin?'是':'否')+' / 記憶封鎖:'+(m.memory_banned?'是':'否')+' / 額度:'+m.quota)+'</span></div>').join('');const st=await getApi('/api/portal/root/state');keyPool.innerHTML='Gemini：'+st.key_pool.gemini_keys+'<br>Vectorize Gemini：'+st.key_pool.vectorize_gemini_keys+'<br>總數：'+st.key_pool.total;rootStats.innerHTML='總對話：'+st.stats.total_calls+'<br>最後模型：'+escapeHtml(st.stats.last_model);errorLogs.innerHTML=(st.errors||[]).slice(-40).reverse().map(e=>'<div class="item"><span>'+escapeHtml(typeof e==='string'?e:JSON.stringify(e))+'</span></div>').join('')||'<div class="muted">目前沒有錯誤日誌</div>'}
    async function loadMatrix(){const q=matrixQuery?.value?('?q='+encodeURIComponent(matrixQuery.value)):"";const res=await getApi('/api/portal/matrix'+q);const particles=res.particles||[];nebulaMessages=particles.map(v=>v.text);p.forEach((dot,i)=>{dot.text=nebulaMessages[i%Math.max(nebulaMessages.length,1)]||'';if(dot.text)dot.h=(i%5)*65+170});matrixList.innerHTML=particles.slice(-60).map(p=>'<div class="item"><span>'+escapeHtml((p.score?'['+p.score+'] ':'')+p.text+(p.raw?' | Array '+JSON.stringify(p.raw):''))+'</span></div>').join('');matrixBox.innerHTML='';particles.slice(0,110).forEach(p=>{const dot=document.createElement('div');dot.title=p.text;dot.style.cssText='position:absolute;width:7px;height:7px;border-radius:50%;background:hsl('+(p.cluster*65+170)+',90%,65%);left:'+Math.random()*98+'%;top:'+Math.random()*94+'%;box-shadow:0 0 14px currentColor';matrixBox.appendChild(dot)})}
    ...
    setQuota.onclick=async()=>{alert((await post('/api/portal/root/member',{qq:rootQQ.value,quota:quotaValue.value||'无限'})).message);loadRoot()}
    ...
    interjectRate.oninput=syncInterjectLabel
