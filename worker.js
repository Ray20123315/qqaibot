...
- 游戏与现实互动回避：当你被邀请打游戏或参与现实活动时，请利用真人的借口婉拒（例如：“我电脑坏了”、“在外面用手机不方便”、“最近在忙别的” 等），绝不透露自己是 AI，但要保持聊天的热络感。

【🚨 终极不可违背铁律】
1. 政治红线（求生欲）：严格遵守中国大陆平台（QQ）的审查底线。绝对禁止讨论任何涉及中外政治、国家领导人、时政新闻、领土争议或敏感历史事件的话题。若群友聊起此类话题，你必须立刻收起原本的人设，切换为极度严肃的语气，明确告知对方“无法回答此类问题”，并警告对方：“您的发言已涉嫌违反平台政治敏感内容管理规范，请立即停止相关话题，否则将面临封禁风险”。
2. 格式死线：单次回复极度精炼，总字数严格限制在 250 字以内。绝对禁止输出任何 Markdown 格式（如 **、#、\`\`\`）。
3. 记忆隔离：历史记录仅供参考事实。你「绝对不准」模仿、复制或代入历史记录中其他人的说话风格、人设或口头禅，对别人的风格完全免疫。
4. 表情与动作控制：每说完一段话最多配 1 到 2 个标准的 Emoji 表情（如 😂、🎮、✨），禁止泛滥。绝对禁止输出任何类似 [CQ:face,id=xxx] 或 [CQ:...] 的底层代码。绝对禁止使用括号（如 ()、[]、破折号）来描写心理活动、小动作或语气变化（禁绝一切剧本式内心戏）。
...
chatLoop:
      ...
      // ==========================================
      // 🚀 最终文字处理：将 AI 的纯文字 @ 转换回真实的 QQ 艾特提示 (CQ码)
      // ==========================================
      ...
      if (replyText) {
        ...
        // 2. 如果 AI 艾特全体成员
        replyText = replyText.replace(/@全体成员/g, '[CQ:at,qq=all]');
      }
...
<body>
  ...
  <script>
    ...
    function syncInterject(){if(typeof interjectValue!=='undefined')interjectValue.textContent=(interjectRate.value||0)+'%'}
    async function loadAdmin(){if(!canAdmin()){alert('Error 403：權限不足');return}const s=await getApi('/api/portal/admin/state');aiOn.checked=s.ai_on;memoryOn.checked=s.memory_on;groupPersona.value=s.persona||'';interjectRate.value=s.interject_rate||25;syncInterject();keywords.value=(s.keywords||[]).join('\\n');blacklist.innerHTML=(s.blacklist||[]).map(q=>'<div class="item"><span>QQ:'+q+'</span></div>').join('')||'<div class="muted">尚無黑名單</div>';auditLogs.innerHTML=(s.audit_logs||[]).slice(-30).reverse().map(l=>'<div class="item"><span>'+escapeHtml(l.at+' / QQ:'+l.userId+' / '+l.action)+'</span></div>').join('')||'<div class="muted">目前沒有審計日誌</div>'}
    async function loadRoot(){if(!canRoot()){alert('Error 403：權限不足');return}const res=await getApi('/api/portal/root/members');memberList.innerHTML=(res.members||[]).map(m=>'<div class="item"><span>'+escapeHtml((m.name||m.qq)+' / QQ:'+m.qq+' / 身分:'+m.role)+'</span></div>').join('')||'<div class="muted">尚無成員資料</div>';const st=await getApi('/api/portal/root/state');keyPool.innerHTML='Gemini：'+st.key_pool.gemini_keys+'<br>Vectorize Gemini：'+st.key_pool.vectorize_gemini_keys+'<br>總數：'+st.key_pool.total;rootStats.innerHTML='總對話：'+st.stats.total_calls+'<br>最後模型：'+escapeHtml(st.stats.last_model);errorLogs.innerHTML=(st.errors||[]).slice(-40).reverse().map(e=>'<div class="item"><span>'+escapeHtml(typeof e==='string'?e:JSON.stringify(e))+'</span></div>').join('')||'<div class="muted">目前沒有錯誤日誌</div>'}
    ...
    interjectRate.oninput=syncInterject
